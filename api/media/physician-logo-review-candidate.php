...
require_once __DIR__.'/../../modules/media/services/PhysicianLogoReviewCandidateService.php';
...
if ($_GET !== [] || $_POST !== [] || array_diff(array_keys($_FILES), ['image']) !== []) candidateReply(400, ['ok'=>false,'error'=>'invalid_request']);
$method = $_SERVER['REQUEST_METHOD'] ?? 'GET';
if (!in_array($method, ['GET','POST','DELETE'], true)) candidateReply(405, ['ok'=>false,'error'=>'method_not_allowed']);
$_SESSION['physician_logo_candidate_csrf'] ??= bin2hex(random_bytes(32));
$token = $_SESSION['physician_logo_candidate_csrf'];
if ($method !== 'GET' && !hash_equals($token, (string)($_SERVER['HTTP_X_PHYSICIAN_LOGO_CANDIDATE_CSRF'] ?? ''))) candidateReply(403, ['ok'=>false,'error'=>'csrf_failed']);
session_write_close();
try {
    $service = new \Media\Services\PhysicianLogoReviewCandidateService(mxmed_pdo(), mxmed_private_media_storage());
    $doctor = $scope['doctor_id'];
    if ($method === 'POST') {
        $upload = $_FILES['image'] ?? [];
        if (!is_array($upload) || !is_string($upload['tmp_name'] ?? null) || !is_string($upload['name'] ?? null) || !is_uploaded_file($upload['tmp_name'])) throw new RuntimeException('candidate_upload_invalid');
    ...
    if ($status === 500) error_log('physician_logo_candidate_failed: '.$error);
