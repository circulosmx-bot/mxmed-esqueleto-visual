...
(function(){
  ...
  function initLegacyContactImport(){
    const card = document.getElementById('mx-dg-contact-card');
    const emailInput = document.getElementById('dp-correo');
    const whatsappInput = document.getElementById('dp-whatsapp');
    const importBox = card?.querySelector('.mx-dg-contact-import');
    const importBtn = document.getElementById('mx-dg-contact-import-btn');
    const feedback = document.getElementById('mx-dg-contact-import-feedback');
    if(!card || !emailInput || !whatsappInput || !importBox || !importBtn || !feedback) return;

    const DEFAULT_DOCTOR_ID = '1';
    const stateClasses = [
      'mx-dg-contact-import--ready',
      'mx-dg-contact-import--loading',
      'mx-dg-contact-import--success',
      'mx-dg-contact-import--warning',
      'mx-dg-contact-import--error'
    ];
    let importing = false;
    ...
    function readFieldValue(input, storageKey){
      const current = clean(input?.value);
      return current || readLocalStorage(storageKey);
    }

    function resolveContactDoctorId(){
      const candidates = [
        (typeof window.resolveDoctorId === 'function' ? window.resolveDoctorId() : ''),
    ...
    function buildImportItems(){
      const items = [];
      const emailValue = readFieldValue(emailInput, 'dp:dp-correo');
      const whatsappValue = readFieldValue(whatsappInput, 'dp:dp-whatsapp');
      if(emailValue){
        items.push({
    ...
    function setBoxState(mode){
      importBox.classList.remove(...stateClasses);
      if(mode){
        importBox.classList.add('mx-dg-contact-import--' + mode);
      }
    }
    ...
    function appendResult(message, tone = 'warning'){
      const row = document.createElement('div');
      row.className = 'mx-dg-contact-import__result mx-dg-contact-import__result--' + (
        tone === 'success' ? 'success' : (tone === 'error' ? 'error' : 'warning')
      );
      row.textContent = clean(message);
    ...
    function setResults(results){
      ...
      if(!list.length){
        setFeedback('No se recibieron resultados de importación.', 'warning');
        setBoxState('warning');
        return;
      }
      ...
      list.forEach((item)=>{
        const legacyKey = clean(item?.legacy_key);
        const type = clean(item?.type);
        const label = legacyKey === 'dp:dp-correo' || type === 'email'
          ? 'Correo privado'
          : 'WhatsApp privado';
        const status = clean(item?.status);
        let message = 'Este dato no pudo procesarse. Revisa la configuración.';
        let tone = 'warning';

        if(status === 'created'){
          message = label + ': guardado como contacto privado.';
          tone = 'success';
        }else if(status === 'already_exists'){
          message = label + ': ya existe como contacto privado.';
          tone = 'success';
        }else if(status === 'invalid_value'){
          message = label + ': revisa el formato antes de importar.';
          tone = 'error';
        }else if(status === 'empty_value'){
          message = label + ': sin valor para importar.';
          tone = 'warning';
        }else if(status === 'duplicate_in_payload'){
          message = label + ': este contacto ya estaba incluido en la solicitud.';
          tone = 'warning';
        }else if(status === 'error'){
          message = label + ': no se pudo importar.';
          tone = 'error';
        }else if(status === 'unsupported_legacy_key' || status === 'unsupported_type' || status === 'mismatch_legacy_key_type'){
          message = label + ': este dato no pudo procesarse. Revisa la configuración.';
          tone = 'error';
        }

        if(tone === 'error') hasError = true;
        if(tone === 'warning') hasWarning = true;
        appendResult(message, tone);
      });
    ...
    function updateImportAvailability(){
      ...
      importBtn.disabled = !hasItems;
      importBtn.textContent = hasItems ? 'Importar contactos privados' : 'Importar contactos privados próximamente';
      if(hasItems){
        setBoxState('ready');
        setFeedback('Puedes importar estos datos como contactos privados.', 'muted');
      }else{
        setBoxState('');
        setFeedback('No hay contactos privados para importar.', 'muted');
      }
    }
    ...
    async function importLegacyContacts(){
      ...
      const confirmed = window.confirm('Se guardarán como contactos privados. No se publicarán ni se copiarán a Consultorio.');
      if(!confirmed) return;
      ...
      setBoxState('loading');
      setFeedback('Importando contactos privados...', 'muted');

      const doctorId = resolveContactDoctorId();
      const endpoint = '/api/profiles/private/doctor/' + encodeURIComponent(doctorId) + '/contact-points/import-legacy';
      ...
      try{
        ...
        if(response.ok && json?.ok === true){
          setResults(results);
        }else if(response.status === 422 && results.length){
          setResults(results);
        }else if(response.status === 400){
          setBoxState('error');
          setFeedback('No se pudo preparar la importación.', 'error');
        }else if(response.status === 422){
          setBoxState('warning');
          setFeedback('Revisa los datos antes de importar.', 'warning');
        }else if(response.status === 500 || response.status === 503){
          setBoxState('error');
          setFeedback('No pudimos importar ahora. Intenta más tarde.', 'error');
        }else{
          setBoxState('error');
          setFeedback('No se pudo importar. Intenta nuevamente.', 'error');
        }
      }catch(_){
        setBoxState('error');
        setFeedback('No pudimos importar ahora. Intenta más tarde.', 'error');
        ...
        importing = false;
        const hasItems = buildImportItems().length > 0;
        importBtn.disabled = !hasItems;
        importBtn.textContent = hasItems ? 'Importar contactos privados' : 'Importar contactos privados próximamente';
      }
    }

    emailInput.addEventListener('input', updateImportAvailability);
    emailInput.addEventListener('change', updateImportAvailability);
    whatsappInput.addEventListener('input', updateImportAvailability);
    whatsappInput.addEventListener('change', updateImportAvailability);
    importBtn.addEventListener('click', importLegacyContacts);
    updateImportAvailability();
  }
  initAutosave();
  initLegacyContactImport();
})();
