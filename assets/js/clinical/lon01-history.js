...
(function () {
  ...
  async function openCanonical(row) {
    ...
    try {
      ...
      if (seen !== epoch || selectedPatient() !== patientId || String(item.patient_id) !== patientId) return;
      ...
      group('Contenido de la consulta', Object.entries({reason_evolution:'Motivo / Evolución', assessment:'Valoración', plan:'Plan', physical_exam:'Exploración'}).filter(([key]) => sections[key]), (target, [key, label]) => {
        target.append(line(label, sections[key].narrative_text || (key === 'physical_exam' ? 'Valores registrados en la exploración' : 'Sin texto')));
        if (key === 'physical_exam' && sections[key].payload?.systems) appendObject(target, sections[key].payload.systems);
      });
      group('Mediciones de esta consulta', item.observations || [], (target, observation) => {
        target.append(line(String(observation.code || 'Medición'), `${observation.systolic_mm_hg != null ? `${observation.systolic_mm_hg}/${observation.diastolic_mm_hg}` : observation.value_numeric ?? 'Sin valor'} ${observation.unit || ''} · ${date(observation.effective_at)} · ${observation.source || 'Origen no disponible'}`));
      });
      group('Documentos y resultados', documentRows, (target, documentRow) => {
        ...
        const timing = kind === 'Resultado' && item.status === 'closed' && documentRow.created_after_final_note != null
          ? late ? ' · Recibido después de finalizar' : ' · Disponible al finalizar' : '';
      ...
      group('Enmiendas del encuentro', item.amendments || [], (target, amendment) => {
        ...
        appendObject(target, amendment.correction || {});
      });
      show(detail, true); detailTitle.focus(); detail.scrollIntoView({block:'nearest'});
      message('Detalle de sólo lectura.');
    } catch (failure) { if (seen === epoch) message(failure.message, true); }
  }
  function openLegacy(row) {
    detailBody.replaceChildren();
    detailTitle.textContent = `Registro anterior${row.status === 'draft' ? ' · Borrador' : ''}`;
  ...
  function card(label, subline, action, className = '') {
    ...
    button.append(title, meta); button.addEventListener('click', () => { detailOrigin = button; action(); });
    return button;
  }
  async function load(reset = false) {
    ...
    if (reset) {
      epoch++; paging = false; patientId = id; offset = 0; seenCanonical.clear(); seenLegacy.clear();
    ...
    try {
      ...
      for (const row of data.canonical || []) {
        ...
        (isOpen ? current : previous).append(card(`Consulta ${state(row.status)}`, `Encuentro canónico · ${date(row.encounter_dt)}`, () => openCanonical(row), isOpen ? 'is-current' : ''));
      }
      for (const row of data.legacy || []) {
        if (seenLegacy.has(row.entry_id)) continue;
        seenLegacy.add(row.entry_id);
        legacy.append(card(`Registro anterior${row.status === 'draft' ? ' · Borrador' : ''}`, `${legacyType(row.note_type)} · fecha de consulta no confirmada`, () => openLegacy(row), 'is-legacy'));
      }
      offset += 25;
  ...
  $('[data-lon01-close]').addEventListener('click', () => { show(detail, false); detailBody.replaceChildren(); (detailOrigin?.isConnected ? detailOrigin : $('[data-lon01-refresh]')).focus(); });
