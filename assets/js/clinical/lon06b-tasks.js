...
(function () {
  ...
  pane.querySelector('[data-lon06b-m7-follow-up]')?.addEventListener('click',async event=>{const trigger=event.currentTarget;if(window.mxmedPlanNextSteps&&!(await window.mxmedPlanNextSteps.mayLeave()))return;const id=Number(pane.querySelector('#m7-workspace [data-m7-body]')?.dataset.encounterId);if(!Number.isSafeInteger(id)||id<1){$('[data-lon06b-error]').textContent='Abre una consulta canónica para vincularla como origen.';show($('[data-lon06b-error]'),true);return;}const tab=pane.querySelector('[data-bs-target="#t-tareas-longitudinal"]');if(tab&&window.bootstrap?.Tab)window.bootstrap.Tab.getOrCreateInstance(tab).show();open('CREATE',null,trigger,{type:'FOLLOW_UP',encounterId:id});});
