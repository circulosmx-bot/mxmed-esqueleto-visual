...
(function () {
  ...
  async function history(row,target) {
    ...
    try {
      ...
      for(const event of data.events||[]) {
        ...
        const changes=[];
        if(before&&before.appointment_id!==after.appointment_id)changes.push(after.appointment_id?`Cita vinculada: ${after.appointment_id}`:'Cita desvinculada');
  ...
  function renderItem(row,target) {
    ...
    meta.textContent=`${stateName[row.state]||row.state} · ${date(row.due_at)} · ${row.source_encounter_id?`Consulta ${row.source_encounter_id}`:'Registro longitudinal'} · ${row.appointment_id?`Cita ${row.appointment_id}`:'Sin cita vinculada'} · versión ${row.row_version}`;
  ...
  function reset(row) {
    $('[data-lon06b-title]').value=row?.title||'';$('[data-lon06b-due]').value=localDate(row?.due_at);$('[data-lon06b-appointment]').value=row?.appointment_id||'';$('[data-lon06b-reason]').value='';
  ...
  function open(operation,row,trigger,context={}) {
    ...
    editing={patientId,operation,row,context,key:crypto.randomUUID()};returnFocus=trigger;reset(row);
    ...
    $('[data-lon06b-context]').textContent=row?`${typeName[row.task_type]} · ${row.title} · ${stateName[row.state]}`:context.encounterId?`Creación explícita desde consulta ${context.encounterId}. La consulta no cambiará.`:'Nueva acción clínica longitudinal.';
  ...
  async function save(event) {
    ...
    if(operation==='CREATE'||operation==='UPDATE') {
      body.title=$('[data-lon06b-title]').value.trim();body.due_at=utcDate($('[data-lon06b-due]').value);body.appointment_id=$('[data-lon06b-appointment]').value.trim()||null;
