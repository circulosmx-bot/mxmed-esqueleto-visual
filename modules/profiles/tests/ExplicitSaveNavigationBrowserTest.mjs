...
try{
  ...
  for(const [width,height] of [[1440,900],[1366,768],[390,844]]){
    ...
    assert.equal(m.right,width<500?29:45);assert.equal(m.bottom,width<500?29:45);assert.equal(m.overflow,false);assert.equal(m.visible,true);assert.equal(m.direction,'column');assert.equal(m.background,'rgb(0, 192, 64)');assert.equal(m.card,'rgba(129, 226, 223, 0.5)');assert.equal(m.modalBackground,'rgb(255, 255, 255)');assert.equal(m.opacity,'1');assert.equal(m.trayRight,width<500?'16px':'32px');assert.equal(m.trayBottom,width<500?'16px':'32px');assert.equal(m.reachable,true);metrics.push(m);
