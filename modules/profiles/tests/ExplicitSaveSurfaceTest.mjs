...
dirty=true;surface.sync({activity:true});assert.equal(last.visible,false);advance(3999);assert.equal(last.visible,false);
surface.sync({activity:true});advance(3999);assert.equal(last.visible,false);advance(1);assert.equal(last.visible,true);
...
console.log('EXPLICIT_SAVE_SURFACE=PASS: deterministic 0/3999/4000 ms, debounce, revert cancellation, stable reminder, scoped navigation, one pending intent, discard without save, retry, duplicate/close lock, clean and beforeunload decisions');
