...
const patient = {first_name: 'Paciente', last_name: 'Sintético', second_last_name: '', mobile_phone: '5550000011', email: 'patient@example.test', birth_day: '1', birth_month: '1', birth_year: '2000', gender: 'F', reason: ''};
...
try {
  for (const [device, width, height] of [['desktop',1440,1100], ['mobile',390,844], ['small',320,740]]) {
    for (const entry of ['direct', 'next']) {
      ...
      assert.ok(await ev(`(()=>{const group=document.querySelector('.mxpp-booking-birth-date');const day=group.querySelector('[data-mxpp-birth-day]');const month=group.querySelector('[data-mxpp-birth-month]');const year=group.querySelector('[data-mxpp-birth-year]');const email=document.querySelector('[name="email"]');const all=[...document.querySelector('[data-mxpp-booking-form]').querySelectorAll('input,select,textarea')];const boxes=[day,month,year].map(e=>e.getBoundingClientRect());const months=[...month.options].slice(1).map(option=>option.textContent);const groupBox=group.getBoundingClientRect();const emailBox=email.closest('label').getBoundingClientRect();const sameDesktopBand=innerWidth<=600||(Math.abs(groupBox.top-emailBox.top)<0.5&&Math.abs(groupBox.bottom-emailBox.bottom)<0.5);return group.querySelector('legend').textContent==='Fecha de nacimiento'&&day.labels[0].textContent==='Día'&&month.labels[0].firstChild.textContent.trim()==='Mes'&&year.labels[0].textContent==='Año'&&day.inputMode==='numeric'&&year.inputMode==='numeric'&&day.autocomplete==='bday-day'&&month.autocomplete==='bday-month'&&year.autocomplete==='bday-year'&&all.indexOf(day)<all.indexOf(month)&&all.indexOf(month)<all.indexOf(year)&&!group.querySelector('input[type="date"]')&&boxes.every(box=>Math.abs(box.width-boxes[0].width)<0.5&&Math.abs(box.height-boxes[0].height)<0.5)&&sameDesktopBand&&month.selectedOptions[0].textContent===''&&JSON.stringify(months)===JSON.stringify(['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'])})()`), 'birth-date selector is accessible, geometrically uniform, band-aligned on desktop, Spanish, and has no visible month placeholder');
