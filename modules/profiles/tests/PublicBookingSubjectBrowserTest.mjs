...
try {
  for (const [device, width, height] of [['desktop',1440,1100], ['mobile',390,844], ['small',320,740]]) {
    for (const entry of ['direct', 'next']) {
      ...
      assert.ok(await ev(`(()=>{const group=document.querySelector('.mxpp-booking-birth-date');const day=group.querySelector('[data-mxpp-birth-day]');const month=group.querySelector('[data-mxpp-birth-month]');const year=group.querySelector('[data-mxpp-birth-year]');const all=[...document.querySelector('[data-mxpp-booking-form]').querySelectorAll('input,select,textarea')];return group.querySelector('legend').textContent==='Fecha de nacimiento'&&day.labels[0].textContent==='Día'&&month.labels[0].firstChild.textContent.trim()==='Mes'&&year.labels[0].textContent==='Año'&&day.inputMode==='numeric'&&year.inputMode==='numeric'&&day.autocomplete==='bday-day'&&month.autocomplete==='bday-month'&&year.autocomplete==='bday-year'&&all.indexOf(day)<all.indexOf(month)&&all.indexOf(month)<all.indexOf(year)&&!group.querySelector('input[type="date"]')})()`), 'birth-date selector is accessible, keyboard ordered, and has no visible native date field');
