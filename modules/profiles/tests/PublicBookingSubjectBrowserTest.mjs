...
try {
  for (const [device, width, height] of [['desktop',1440,1100], ['mobile',390,844], ['small',320,740]]) {
    for (const entry of ['direct', 'next']) {
      ...
      assert.ok(await ev(`(()=>{const group=document.querySelector('.mxpp-booking-patient-type');const options=[...group.querySelectorAll('[data-mxpp-booking-patient-type]')];return group.querySelector('legend').textContent==='¿Es la primera vez que este paciente consulta con este especialista?'&&options.length===2&&options.map(option=>option.value).join(',')==='first_time,follow_up'&&options.every(option=>option.type==='radio')})()`), 'other-person patient-type selector is accessible and refers to the patient');
      ...
      assert.ok(await ev(`document.querySelector('#mxpp-booking-patient-title').textContent==='Reserva de cita'&&document.querySelector('#mxpp-booking-patient-section-title').textContent==='Datos del paciente'&&document.querySelector('[data-mxpp-booker-fields]').hidden&&document.querySelector('[data-mxpp-booking-submit]').textContent==='Continuar con la reserva'&&document.querySelector('.mxpp-booking-patient-type legend').textContent==='¿Es la primera vez que consultas con este especialista?'&&document.querySelector('[data-mxpp-booking-patient-type]:checked').value==='first_time'`), 'self hierarchy keeps the patient declaration and has no requester section');
