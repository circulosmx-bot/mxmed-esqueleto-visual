...
try {
  for (const [device, width, height] of [['desktop',1440,900], ['compact',1366,768], ['mobile',390,844], ['small',320,740]]) {
    for (const entry of ['direct', 'next']) {
      ...
      assert.ok(await ev(`(()=>{const root=document.querySelector('[data-mxpp-booking-step="patient"]');const summary=getComputedStyle(root.querySelector('.mxpp-booking-modal__summary'));const summaryText=getComputedStyle(root.querySelector('.mxpp-booking-modal__summary p'));const doctor=root.querySelector('.mxpp-booking-modal__doctor-summary');const icon=doctor.querySelector('.material-symbols-rounded');const patientHeading=getComputedStyle(document.querySelector('#mxpp-booking-patient-section-title'));const bookerHeading=getComputedStyle(document.querySelector('[data-mxpp-booker-fields] legend'));const cta=getComputedStyle(document.querySelector('[data-mxpp-booking-submit]'));return summary.backgroundColor==='rgb(1, 175, 183)'&&summaryText.color==='rgb(255, 255, 255)'&&parseFloat(summaryText.fontSize)>=18&&doctor.textContent.trim()==='stethoscopeDra. Leticia Muñoz Romo'&&!doctor.textContent.includes('Doctor:')&&icon.textContent.trim()==='stethoscope'&&icon.getAttribute('aria-hidden')==='true'&&parseFloat(patientHeading.fontSize)>=19&&parseFloat(bookerHeading.fontSize)>=19&&cta.backgroundColor==='rgb(1, 175, 183)'&&cta.color==='rgb(255, 255, 255)'})()`), 'summary has icon and doctor name without the redundant label while preserving the visual contract');
