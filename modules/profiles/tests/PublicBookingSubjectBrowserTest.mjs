...
try {
  for (const [device, width, height] of [['desktop',1440,1100], ['mobile',390,844], ['small',320,740]]) {
    for (const entry of ['direct', 'next']) {
      ...
      assert.ok(await ev(`(()=>{const group=document.querySelector('.mxpp-booking-birth-date');const day=group.querySelector('[data-mxpp-birth-day]');const month=group.querySelector('[data-mxpp-birth-month]');const year=group.querySelector('[data-mxpp-birth-year]');const all=[...document.querySelector('[data-mxpp-booking-form]').querySelectorAll('input,select,textarea')];const boxes=[day,month,year].map(e=>e.getBoundingClientRect());const months=[...month.options].slice(1).map(option=>option.textContent);return group.querySelector('legend').textContent==='Fecha de nacimiento'&&day.labels[0].textContent==='Día'&&month.labels[0].firstChild.textContent.trim()==='Mes'&&year.labels[0].textContent==='Año'&&day.inputMode==='numeric'&&year.inputMode==='numeric'&&day.autocomplete==='bday-day'&&month.autocomplete==='bday-month'&&year.autocomplete==='bday-year'&&all.indexOf(day)<all.indexOf(month)&&all.indexOf(month)<all.indexOf(year)&&!group.querySelector('input[type="date"]')&&boxes.every(box=>Math.abs(box.width-boxes[0].width)<0.5&&Math.abs(box.height-boxes[0].height)<0.5)&&month.selectedOptions[0].textContent===''&&JSON.stringify(months)===JSON.stringify(['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'])})()`), 'birth-date selector is accessible, equal-width, keyboard ordered, Spanish, and has no visible month placeholder');
