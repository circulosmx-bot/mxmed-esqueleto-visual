...
try{
  ...
  for(const [width,height] of [[1440,900],[1366,768],[390,844]]){
    ...
    swatches.forEach((s,i)=>{
      ...
      assert.equal(s.color,rgb(catalog[i].accent));assert.equal(s.width,38);assert.equal(s.height,20);assert.equal(s.radius,5);
    ...
    if(width===1440)await screenshot('crd037-theme-rectangles-1440.png');
    if(width===390)await screenshot('crd037-theme-rectangles-mobile.png');
    ...
    if(width===1440)await screenshot('crd037-theme-rectangles-selected.png');
  ...
  await writeFile(output+'/metrics.json',JSON.stringify({admin:metrics,public:publicResults},null,2));
  console.log('CRD037_BROWSER=PASS: split 15.096/17.76px exact blue; copy; 38x20/5px swatches; 44px targets; labels/catalog; ring/check/focus; keyboard; preview/reset; mocked grouped PATCH; 60 public theme previews; three viewports');
