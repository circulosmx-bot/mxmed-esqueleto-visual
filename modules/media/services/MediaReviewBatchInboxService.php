...
final class MediaReviewBatchInboxService
{
    ...
    private const SUMMARY="SELECT b.batch_id,b.owner_type,b.owner_id,p.display_name owner_display_name,b.status,b.opened_at,b.last_activity_at,b.submitted_at,MIN(COALESCE(s.submitted_for_review_at,b.submitted_at)) first_submitted_at,COUNT(s.submission_id) item_count,SUM(s.review_status='PENDING_REVIEW') pending_count,SUM(s.review_status='APPROVED') approved_count,SUM(s.review_status='NEEDS_WORK') needs_work_count,SUM(s.purpose='DOCTOR_PROFILE_PHOTO') photo_count,SUM(s.purpose='PHYSICIAN_PERSONAL_LOGO') logo_count,SUM(s.purpose='DOCTOR_GALLERY') gallery_count FROM media_review_batches b JOIN media_review_submissions s ON s.batch_id=b.batch_id LEFT JOIN profiles_doctors p ON p.doctor_id=b.owner_id ";
    ...
    public function listing(AuthorizationContext|TrustedAuthorizationContext|null $context,int $limit=25,int $offset=0):array
    {
        ...
        $s=$this->pdo->prepare(self::SUMMARY."WHERE ".MediaSubmissionState::sql()." GROUP BY b.batch_id,p.display_name HAVING pending_count>0 ORDER BY first_submitted_at,b.batch_id LIMIT ? OFFSET ?");
