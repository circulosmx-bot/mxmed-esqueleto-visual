...
final class LogoImprovementService
{
    ...
    public function mutate(?TrustedAuthorizationContext $context,string $action,string $id):array
    {
        ...
        try {
            ...
            $prepare=function()use($action,$id,&$new,&$old,&$temps,&$result):array{
                [$candidate,$files]=$this->resolve($id,true);$input=$files['IMPROVEMENT_INPUT']??null;$proposal=$files['AUTO_PROPOSAL']??null;
                if($input===null&&$action!=='discard'){if($action==='generate')throw new RuntimeException('IMPROVEMENT_INPUT_UNAVAILABLE');throw new RuntimeException('improvement_conflict');}
                if($action==='generate'){
                    ...
                    $output=(new GdPublicLogoProcessor(4194304,800,640000))->process(['tmp_name'=>$path,'type'=>$input['mime_type'],'error'=>0],false,null,
                    ...
                    $temps[]=$output['path'];
                    if(hash_equals($files['REVIEW']['checksum_sha256'],$output['checksum_sha256']))throw new RuntimeException('ALREADY_IMPROVED');
                    $file=$this->newFile($candidate,'AUTO_PROPOSAL',$output);
            ...
            if($safe&&in_array($e->getMessage(),['NO_SAFE_IMPROVEMENT','ALREADY_TRANSPARENT','ALREADY_IMPROVED','IMPROVEMENT_INPUT_UNAVAILABLE'],true))return ['ok'=>true,'status'=>$e->getMessage(),'submission_id'=>$id];
            throw $e;
    ...
    public function preview(?TrustedAuthorizationContext $context,string $id):string
    {
        ...
        [$candidate,$files]=$this->resolve($id,false);$proposal=$files['AUTO_PROPOSAL']??null;$input=$files['IMPROVEMENT_INPUT']??null;
        if(!$input||!$proposal||$proposal['input_file_id']!==$input['file_id']||$proposal['input_checksum_sha256']!==$input['checksum_sha256'])throw new RuntimeException('improvement_conflict');
    ...
    private function resolve(string $id,bool $lock):array
    {
        ...
        if(!isset($files['REVIEW']))throw new RuntimeException('improvement_integrity_failed');return [$candidate,$files];
    }
    private function verified(array $file):string
    {
        $o=$this->storage->openReadStream($file['storage_key']);try{$bytes=stream_get_contents($o['stream'],$file['role']==='IMPROVEMENT_INPUT'?4194305:153601);}finally{fclose($o['stream']);}
