...
final class ProfilePhotoReviewCandidateService
{
    ...
    {
        $s = $this->pdo->prepare("SELECT submission_id,technical_status,review_status,created_at,updated_at FROM media_review_submissions WHERE owner_type='PHYSICIAN' AND owner_id=? AND purpose='DOCTOR_PROFILE_PHOTO' AND review_status='PENDING_REVIEW'");
        $s->execute([$doctor]);
    ...
    public function upload(string $doctor, array $upload): void
    {
        ...
        try {
            ...
            $review = $processor->process($upload, false, function(string $source, string $mime, int $width, int $height, int $bytes) use (&$files, &$newKeys, $prefix): void {
                ...
                $newKeys[] = $key;
                $this->storage->storeImmutable($key, $source);
            ...
            $newKeys[] = $key;
            $this->storage->storeImmutable($key, $review['path']);
            $files[] = array_merge($review, ['file_id'=>$fileId,'role'=>'REVIEW','storage_key'=>$key]);
            $this->switchPending($doctor, $id, $files);
        } catch (\Throwable $e) {
            $this->cleanup($newKeys);
            throw $e;
        } finally {
    ...
    private function switchPending(string $doctor, ?string $id, array $files): void
    {
        $oldKeys = [];
        try {
            ...
            $s = $this->pdo->prepare("SELECT submission_id FROM media_review_submissions WHERE owner_type='PHYSICIAN' AND owner_id=? AND purpose='DOCTOR_PROFILE_PHOTO' AND review_status='PENDING_REVIEW' FOR UPDATE");
            $s->execute([$doctor]);
            ...
            if ($id !== null) {
                $s = $this->pdo->prepare("INSERT INTO media_review_submissions(submission_id,owner_type,owner_id,purpose,technical_status,review_status) VALUES(?,'PHYSICIAN',?,'DOCTOR_PROFILE_PHOTO','READY','PENDING_REVIEW')");
                $s->execute([$id, $doctor]);
