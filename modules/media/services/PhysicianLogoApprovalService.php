...
final class PhysicianLogoApprovalService
{
    ...
    public function approve(?TrustedAuthorizationContext $context, string $id): array
    {
        ...
        try {
            ...
            $audit = new MediaReviewAudit($this->pdo,$context,function() use ($id,$mediaId,&$candidate,&$review,&$bytes,&$old): array {
                [$candidate,$review,$bytes] = $this->lockAndVerify($id);
                $s=$this->pdo->prepare("SELECT media_id,storage_key FROM media_assets WHERE owner_type='PHYSICIAN' AND owner_id=? AND purpose='PHYSICIAN_PERSONAL_LOGO' AND status='READY' FOR UPDATE");
            ...
            },'MEDIA_PHYSICIAN_LOGO_APPROVED',self::CAPABILITY,'POST /api/internal/media-review/approve-logo.php');
            ...
            $proposedKey='public/physician-personal-logo/'.hash('sha256','PHYSICIAN:'.$candidate['owner_id']).'/'.$mediaId.'.webp';
            ...
            $repository->insertReady(array_merge($review,['media_id'=>$mediaId,'owner_type'=>'PHYSICIAN','owner_id'=>$candidate['owner_id'],
                'purpose'=>'PHYSICIAN_PERSONAL_LOGO','storage_key'=>$key,'public_url'=>$url,'alt_text'=>'Logotipo profesional']));
            $s=$this->pdo->prepare('UPDATE profiles_doctors SET logo_url=?,updated_at=CURRENT_TIMESTAMP WHERE doctor_id=?');
            $s->execute([$url,$candidate['owner_id']]);if ($s->rowCount()!==1) throw new RuntimeException('approval_profile_switch_failed');
            // Historical documents can retain old URLs outside canonical profile references.
            // Keep old immutable READY rows/files; never rewrite or invalidate snapshots.
    ...
    private function lockAndVerify(string $id): array
    {
        ...
        $s=$this->pdo->prepare('SELECT * FROM media_review_files WHERE submission_id=? AND role<>\'AUTO_PROPOSAL\' ORDER BY role FOR UPDATE');$s->execute([$id]);$files=$s->fetchAll(PDO::FETCH_ASSOC);
