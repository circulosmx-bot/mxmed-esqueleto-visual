...
final class GalleryApprovalService
{
    ...
    public function approve(?TrustedAuthorizationContext $context, string $id): array
    {
        ...
        $key = null;$temporary = null;$commitAttempted = false;$candidate = null;$review = null;$bytes = null;
        ...
        try {
            ...
            $audit = new MediaReviewAudit($this->pdo,$context,function() use ($id,$mediaId,&$candidate,&$review,&$bytes): array {
                [$candidate,$review,$bytes] = $this->lockAndVerify($id);
                GalleryCapacity::requireApprovalSlot($this->pdo,(string)$candidate['owner_id']);
                return ['submission_id'=>$id,'physician_id'=>(string)$candidate['owner_id'],'published_media_id'=>$mediaId];
            },'MEDIA_DOCTOR_GALLERY_APPROVED',self::CAPABILITY,'POST /api/internal/media-review/approve-gallery.php');
            ...
            $proposedKey='public/doctor-gallery/'.hash('sha256','PHYSICIAN:'.$candidate['owner_id']).'/'.$mediaId.'.webp';
            ...
            $repository->insertReady(array_merge($review,['media_id'=>$mediaId,'owner_type'=>'PHYSICIAN','owner_id'=>$candidate['owner_id'],
                'purpose'=>'DOCTOR_GALLERY','storage_key'=>$key,'public_url'=>$url,'alt_text'=>'Foto de galería']));
    ...
    private function lockAndVerify(string $id): array
    {
        ...
        if (!$candidate || (string)$candidate['owner_id']!==(string)$owner || $candidate['owner_type']!=='PHYSICIAN'
            || $candidate['purpose']!=='DOCTOR_GALLERY' || $candidate['technical_status']!=='READY'
