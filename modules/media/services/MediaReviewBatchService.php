...
require_once __DIR__.'/MediaCandidateSubmissionService.php';
use PDO;
...
final class MediaReviewBatchService
{
    ...
    public function retireEmptyOpenLocked(string $doctor,string $withdrawnId): bool
    {
        ...
        $s=$this->pdo->prepare('SELECT submission_id FROM media_review_submissions WHERE batch_id=? AND submitted_for_review_at IS NOT NULL FOR UPDATE');$s->execute([$id]);
    ...
    {
        $s=$this->pdo->prepare("SELECT b.opened_at,b.last_activity_at,DATE_ADD(b.last_activity_at,INTERVAL ".self::MEDIA_REVIEW_BATCH_INACTIVITY_MINUTES." MINUTE) auto_submit_after,(SELECT COUNT(*) FROM media_review_submissions s WHERE s.batch_id=b.batch_id AND s.technical_status='READY' AND s.review_status='PENDING_REVIEW' AND s.submitted_for_review_at IS NULL) item_count,(SELECT COUNT(*) FROM media_review_submissions s WHERE s.batch_id=b.batch_id AND s.technical_status='READY' AND s.review_status='PENDING_REVIEW' AND s.submitted_for_review_at IS NULL) pending_count FROM media_review_batches b WHERE b.owner_type='PHYSICIAN' AND b.owner_id=? AND b.status='OPEN'");
    ...
    public function submit(string $doctor,?string $eligibleId=null,?MediaSubmissionActor $actor=null): bool
    {
        if($actor===null || $actor->doctor!==$doctor || ($actor->system !== ($eligibleId!==null)))throw new RuntimeException('submission_unauthorized');
        ...
        try {
            ...
            $s=$this->pdo->prepare("SELECT * FROM media_review_submissions WHERE batch_id=? AND technical_status='READY' AND review_status='PENDING_REVIEW' AND submitted_for_review_at IS NULL ORDER BY submission_id FOR UPDATE");$s->execute([$row['batch_id']]);
            $items=$s->fetchAll(PDO::FETCH_ASSOC);
            // An all-sent OPEN session is closed by the executor, not shown as pending.
            if(!$items && $eligibleId===null){$this->pdo->rollBack();return false;}
            $submission=new MediaCandidateSubmissionService($this->pdo);
            foreach($items as $item)$submission->submitLocked($item,$actor,$eligibleId===null?'BATCH':'INACTIVITY');
