    ...
    public static function run(bool $options=false, bool $logo=false): void
    {
        ...
        try {
            ...
            if ($options) {
                ...
                $canImprove=$caps->contains('media_review_improve');
                $canReplace=$caps->contains('media_review_request_replacement');
                $canDownload=$caps->contains('media_review_source_download');$canCorrect=$caps->contains('media_review_corrected_upload');
                self::respond(200,['ok'=>true,'can_approve'=>$canApprove,'can_download_source'=>$canDownload,'can_upload_corrected'=>$canCorrect,'can_improve'=>$canImprove,'can_request_replacement'=>$canReplace,
                    'csrf'=>($canApprove||$canCorrect||$canImprove||$canReplace)?$identity->csrf()->issueAuthenticated($session->session()->tokenDigest()):null]);return;
            }
