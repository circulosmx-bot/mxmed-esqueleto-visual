...
$u=mr8Upload('white',5000,5000);$status=null;
try{$out=(new Media\Services\GdPublicLogoProcessor(10485760,8192,25000000))->process($u,false,null,function($im)use($engine,&$status){$status=$engine->apply($im);});check($status==='PROPOSAL_CREATED'&&max($out['width'],$out['height'])<=800&&$out['byte_size']<=153600,'25MP_bounded_processing');unlink($out['path']);}finally{unlink($u['tmp_name']);}
