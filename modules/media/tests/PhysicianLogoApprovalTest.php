...
use Media\Services\PhysicianLogoApprovalService;
...
$p=mr5Pdo();[$private,$public]=mr5Storage();$service=new PhysicianLogoApprovalService($p,$private,$public);
$f=mr7Candidate($p);$before=state($p,$f);$files=publicFiles();
...
foreach (['missing','checksum','length','store','media_insert','profile_update','candidate_update','audit_insert','audit_cas','commit'] as $failure) {
    $f=mr7Candidate($p);$before=state($p,$f);$files=publicFiles();$trigger=null;
    ...
    $sut=new PhysicianLogoApprovalService($db,in_array($failure,['missing','checksum','length'],true)?new BrokenPrivate($private,$failure):$private,$failure==='store'?new BrokenPublic($public,'store'):$public);
...
foreach (['APPROVED','WITHDRAWN','REJECTED','NEEDS_WORK'] as $status) {
    $f=mr7Candidate($p);$p->prepare('UPDATE media_review_submissions SET review_status=? WHERE submission_id=?')->execute([$status,$f['id']]);$before=state($p,$f);
...
foreach (['normal','cleanup'] as $mode) {
    $f=mr7Candidate($p);$s=$p->prepare("SELECT * FROM media_review_files WHERE submission_id=? ORDER BY role");$s->execute([$f['id']]);$history=$s->fetchAll();
    $sut=new PhysicianLogoApprovalService($p,$private,$mode==='cleanup'?new BrokenPublic($public,'cleanup'):$public);
    ...
    $s=$p->prepare('SELECT logo_url FROM profiles_doctors WHERE doctor_id=?');$s->execute([$f['doctor']]);check($s->fetchColumn()===$result['public_url'],'profile_canonical_url');
    $s=$p->prepare("SELECT * FROM media_assets WHERE owner_id=? AND status='READY' AND classification='PUBLIC'");$s->execute([$f['doctor']]);$rows=$s->fetchAll();check(count($rows)===2,'historical_and_current_assets_retained');$rows=array_values(array_filter($rows,fn($r)=>$r['public_url']===$result['public_url']));check(count($rows)===1,'one_current_canonical_asset');
...
$f=mr7Candidate($p);$db=new LostCommitAcknowledgementPdo('mysql:host=127.0.0.1;port=3309;dbname=mxmed','root','',[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION]);
try{(new PhysicianLogoApprovalService($db,$private,$public))->approve(mr5Context(),$f['id']);throw new LogicException('unexpected acknowledgement');}
...
echo "PHYSICIAN_LOGO_APPROVAL_SERVICE_QA=PASS\n";
