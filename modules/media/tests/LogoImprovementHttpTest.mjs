...
try {
 ...
 for(const query of ['&role=SOURCE','&path=forged','&storage_key=forged'])assert.equal((await request('improvement-preview.php?submission_id='+f.id+query)).status,400);
 for(const key of [proposal.file_id,proposal.storage_key,encodeURIComponent(proposal.storage_key)])assert.equal((await fetch(base+'/api/media/index.php/public/'+key)).status,404);
