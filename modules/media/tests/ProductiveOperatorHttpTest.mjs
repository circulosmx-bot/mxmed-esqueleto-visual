...
async function request(route,token,suffix='',extra={},method='GET'){
 const r=await fetch(`${base}/api/internal/media-review/${route}.php?${route==='pending'?'':'submission_id='+id}${suffix}`,{method,headers:{...(token?{Cookie:'__Host-mxmed_session='+token}:{}),...extra},...(method==='POST'?{body:JSON.stringify({account_id:'mr3_good',capability:'media_review_read'})}:{})});
...
try{
 ...
 for(const route of ['submission','review-image','pending']){
 ...
 for(const key of [null,'customer','wrong','revoked_grant','inactive','expired','revoked_session','superseded','credential']){
  const r=await fetch(base+'/internal/media-review/',{headers:key?{Cookie:'__Host-mxmed_session='+setup.tokens[key]}:{}});assert.equal(r.status,403);const text=await r.text();assert.ok(!text.includes(id)&&!text.includes('pending-cards'));
 }
 const page=await fetch(base+'/internal/media-review/',{headers:{Cookie:'__Host-mxmed_session='+setup.tokens.good}});assert.equal(page.status,200);assert.ok((await page.text()).includes('Revisión de medios'));
 const list=await request('pending',setup.tokens.good);assert.equal(list.status,200,list.bytes.toString());assert.equal(JSON.parse(list.bytes).data.items.filter(item=>item.submission_id===id).length,1);
 ...
 fixture('revoke');assert.equal((await request('pending',setup.tokens.good)).status,403);assert.equal((await request('submission',setup.tokens.good)).status,403);assert.equal((await request('review-image',setup.tokens.good)).status,403);
