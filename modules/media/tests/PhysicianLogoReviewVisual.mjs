...
export async function runLogoVisualQA({base,tokens,candidate,sql,upload,grant,revoke}) {
 const output=process.env.MR7_VISUAL_DIR||'/tmp/mxmed-mr7-visual';await mkdir(output,{recursive:true});
 ...
 try {
  ...
  for(const [width,height] of [[1440,900],[1366,768],[390,844],[320,740]]) {
   for(const [shape,w,h] of [['horizontal',900,300],['square',500,500],['tall',240,900],['transparent',700,400]]) {
   const fixture=candidate(w,h);
   ...
   assert.equal(await evaluate("document.getElementById('detail-purpose').textContent"),'Logotipo profesional');
   assert.equal(await evaluate("getComputedStyle(document.getElementById('detail-image')).objectFit"),'contain');
   assert.ok(await evaluate("getComputedStyle(document.querySelector('.detail-image-wrap')).backgroundImage.includes('gradient')"));
   assert.equal(await evaluate("(()=>{const i=document.getElementById('detail-image'),c=document.createElement('canvas');c.width=i.naturalWidth;c.height=i.naturalHeight;const x=c.getContext('2d');x.drawImage(i,0,0);return x.getImageData(0,0,1,1).data[3]<30})()"),true,'visible retained alpha');
   await screenshot(width+'-'+shape+'-selected');
   ...
   await screenshot(width+'-'+shape+'-busy');
   ...
   await screenshot(width+'-'+shape+'-corrected');
   ...
   await until("document.getElementById('inbox-message').textContent==='Logotipo aprobado y publicado.'");
   ...
   await screenshot(width+'-'+shape+'-published');
  }
  }
  candidate();await send('Page.navigate',{url:base+'/internal/media-review/'});await until("document.querySelector('.review-thumb img')?.naturalWidth===800");await evaluate("document.querySelector('.card button').click()");await until("document.getElementById('detail-image').naturalWidth===800 && !document.getElementById('choose-corrected').hidden");
  ...
  console.log('MR7_VISUAL_QA=PASS: four viewports, audited SOURCE download, native file picker, busy guard, refreshed REVIEW, MR5 approval, keyboard, capability-specific controls, no overflow; '+output);
 } catch(error) {
