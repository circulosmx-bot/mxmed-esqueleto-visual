...
export async function ownerBrowser({base,image,owner,reviewer}) {
 const profile=await mkdtemp('/tmp/mxmed-mr12a-browser-');
 const chrome=spawn(process.env.MXMED_QA_CHROME||'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome',['--headless=new','--no-first-run','--no-default-browser-check','--remote-debugging-address=127.0.0.1','--remote-debugging-port=9348','--user-data-dir='+profile],{stdio:'ignore'});
 let ws;
 try{
  let version;for(let i=0;i<100;i++){try{version=await(await fetch('http://127.0.0.1:9348/json/version')).json();break;}catch{await new Promise(r=>setTimeout(r,100));}}
  ...
  await send('Network.setCookie',{name:'PHPSESSID',value:owner,url:base,path:'/'});
  await send('Page.navigate',{url:base+'/index.html'});await until('Boolean(window.mxmedMediaReview && document.getElementById("mxpi-photo-input"))');
  ...
  await send('Network.setCookie',{name:'__Host-mxmed_session',value:reviewer,url:base,path:'/',secure:true,httpOnly:true});
  await send('Page.navigate',{url:base+'/internal/media-review/'});await until('document.querySelectorAll("#batch-cards button").length===1');
