...
$sql = (string)($create[1] ?? '');
foreach (['media_id', 'owner_type', 'owner_id', 'purpose', 'classification', 'storage_key', 'public_url', 'mime_type', 'width', 'height', 'byte_size', 'checksum_sha256', 'alt_text', 'display_order', 'status'] as $field) {
    mediaMigrationAssert(str_contains($sql, '`' . $field . '`'), 'database field present: ' . $field);
}
mediaMigrationAssert(str_contains($sql, "enum('PHYSICIAN_PERSONAL_LOGO','CONSULTORIO_GROUP_LOGO','DOCTOR_GALLERY','DOCTOR_PROFILE_PHOTO')"), 'database purpose enum exact');
...
mediaMigrationAssert(str_contains($sql, 'chk_media_assets_display_order'), 'database gallery display-order constraint active');
mediaMigrationAssert(str_contains($sql, 'idx_media_assets_gallery_order'), 'database gallery display-order read index active');
