...
try{
 if($mode==='corrected'){$upload=mr8Upload('offwhite');try{$result=(new Media\Services\MediaReviewInterventionService(mr5Pdo(),new Mr6PausingStorage($private,$gate)))->corrected(mr6Context('upload_corrected',['media_review_corrected_upload']),$id,$upload);}finally{unlink($upload['tmp_name']);}}
 elseif($mode==='approve')$result=(new Media\Services\PhysicianLogoApprovalService(mr5Pdo(),$private,new Mr6PausingStorage($public,$gate)))->approve(mr5Context(),$id);
 elseif(in_array($mode,['generate','accept'],true))$result=(new Media\Services\LogoImprovementService(mr5Pdo(),new Mr6PausingStorage($private,$gate)))->mutate(mr8Context($mode),$mode,$id);
