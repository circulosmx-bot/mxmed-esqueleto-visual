...
for(const [first,second] of [['accept','approve'],['approve','accept'],['corrected','accept'],['approve','generate'],['generate','approve'],['corrected','generate'],['generate','corrected']]){
 ...
 try{
  ...
  if(first==='approve'||(first==='corrected'&&second==='accept')){assert.equal(two.code,2);assert.equal(two.error,'improvement_conflict');}
  else if(second==='corrected'){assert.equal(two.code,0,JSON.stringify(two));assert.equal(sql(`echo $p->query("SELECT COUNT(*) FROM media_review_files WHERE submission_id='${f.id}' AND role='AUTO_PROPOSAL'")->fetchColumn();`),'0');}
  else if(first==='corrected'&&second==='generate'){assert.equal(two.code,0,JSON.stringify(two));assert.equal(sql(`$f=mr6Rows($p,'${f.id}');echo (int)($f['AUTO_PROPOSAL']['input_file_id']===$f['IMPROVEMENT_INPUT']['file_id']&&$f['AUTO_PROPOSAL']['input_checksum_sha256']===$f['IMPROVEMENT_INPUT']['checksum_sha256']);`),'1');}
