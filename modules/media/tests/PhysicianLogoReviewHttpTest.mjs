...
import {runLogoVisualQA} from './PhysicianLogoReviewVisual.mjs';
import {runLogoOwnerQA} from './PhysicianLogoOwnerHttpQA.mjs';
const fixtureRoot=process.env.MR5_FIXTURE_ROOT||'/tmp/mxmed-mr5-mr7qa';
const env={...process.env,MR5_FIXTURE_ROOT:fixtureRoot,MR3_TEST_DB:'mxmed_gate4d_preview_mr3_mr7_'+Date.now(),MR3_TEST_DB_HOST:'127.0.0.1',MR3_TEST_DB_PORT:'3309',MR3_TEST_DB_USER:'root',MR3_TEST_DB_PASS:'',MR3_VALKEY_PORT:'6387'};
...
const candidate=(w=900,h=300)=>JSON.parse(execFileSync('php',['modules/media/tests/PhysicianLogoReviewFixture.php','candidate','Logotipo sintético MR7',String(w),String(h)],{env,encoding:'utf8'}));
...
try {
 ...
 root=await mkdtemp('/tmp/mxmed-mr7-http-');
 ...
 assert.equal(response.headers.get('cache-control'),'private, no-store');assert.equal(response.headers.get('x-content-type-options'),'nosniff');assert.match(response.headers.get('content-disposition'),/^attachment; filename="logo-original-[a-f0-9]{8}\.png"$/);assert.notEqual(response.headers.get('access-control-allow-origin'),'*');
 ...
 assert.equal((await request('approve.php','good',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({submission_id:f.id,csrf:opts.csrf})})).status,409,'photo endpoint rejects logo');
 const approval=await post(f.id,'good',opts.csrf);assert.equal(approval.status,200);const published=await approval.json();
 assert.equal((await fetch(base+f.old.public_url)).status,200,'historical logo remains public');
 assert.equal(sql(`echo $p->query("SELECT logo_url FROM profiles_doctors WHERE doctor_id='${f.doctor}'")->fetchColumn();`),published.public_url);
 ...
 await runLogoOwnerQA({base,php,sql,candidate,request,options,post,env});
 ...
 await runLogoVisualQA({base,tokens:identity.tokens,candidate,sql,upload,grant,revoke});
 await rm(upload.tmp_name,{force:true});
 assert.equal(snapshot(),before,'Director baseline changed');
 console.log('MR7_HTTP_QA=PASS: SOURCE exact attachment/no-store, explicit capability/session/CSRF denials, correction/replacement, private-only state, audit failure zero bytes/rollback, MR5 new REVIEW publication; Director unchanged');
} finally {
