...
const run=mode=>JSON.parse(execFileSync('php',['modules/media/tests/ProfilePhotoApprovalFixture.php',mode],{env,encoding:'utf8'}));
...
const worker=(mode,id,gate='')=>{const p=spawn('php',['modules/media/tests/MediaReviewInterventionRaceWorker.php',mode,id,gate],{env,stdio:['ignore','pipe','pipe']});let out='',error='';p.stdout.on('data',b=>out+=b);p.stderr.on('data',b=>error+=b);return new Promise(resolve=>p.on('exit',code=>resolve({code,...JSON.parse(out||'{}'),stderr:error})));};
for(const first of ['corrected','approve']){
 ...
 try{
  ...
  if(first==='corrected'){
   ...
   const row=JSON.parse(query(`echo json_encode($p->query("SELECT (SELECT checksum_sha256 FROM media_review_files WHERE submission_id='${f.id}' AND role='REVIEW') review_hash,(SELECT checksum_sha256 FROM media_assets WHERE owner_id='${f.doctor}' AND status='READY') public_hash")->fetch());`));
  ...
  assert.equal(query(`echo $p->query("SELECT COUNT(*) FROM media_assets WHERE owner_id='${f.doctor}' AND status='READY'")->fetchColumn();`),'1');
