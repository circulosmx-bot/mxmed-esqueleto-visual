...
$p=mr5Pdo();[$private,$public]=mr5Storage();$f=mr7Candidate($p,'Logotipo sintético MR7',900,300,false);$doctor=$f['doctor'];$id=$f['id'];
...
(new Media\Services\MediaReviewBatchService($p))->submit($doctor,null,mr11Actor($doctor));
...
mr11Submit($p,$doctor);$id=$pending['submission_id'];$original=mr6Rows($p,$id)['SOURCE'];$intervention=new Media\Services\MediaReviewInterventionService($p,$private);
