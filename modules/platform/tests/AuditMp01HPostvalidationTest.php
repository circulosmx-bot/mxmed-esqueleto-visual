...
h01Ok($tests, count($events) === 44 && count(array_unique($events)) === 44, 'canonical_event_count');
h01Ok($tests, $policyEvents === $events, 'policy_catalog_exact_order');
h01Ok($tests, count($rows) === 44, 'policy_row_count');
...
h01Ok($tests, array_merge($combined,['INTERNAL_GOVERNANCE_CHANGED','MEDIA_PROFILE_PHOTO_APPROVED','MEDIA_REVIEW_SOURCE_DOWNLOADED','MEDIA_REVIEW_CORRECTED_UPLOADED','MEDIA_PHYSICIAN_LOGO_APPROVED','MEDIA_LOGO_IMPROVEMENT_PROPOSED','MEDIA_LOGO_IMPROVEMENT_ACCEPTED','MEDIA_LOGO_IMPROVEMENT_DISCARDED','MEDIA_REVIEW_REPLACEMENT_REQUESTED','MEDIA_DOCTOR_GALLERY_APPROVED','PHYSICIAN_CREDENTIAL_PROVISIONED','PHYSICIAN_CREDENTIAL_VERIFIED','PHYSICIAN_CREDENTIAL_REJECTED','PHYSICIAN_CREDENTIAL_INACTIVATED','PHYSICIAN_CREDENTIAL_REVOKED','MEDIA_REVIEW_CANDIDATE_SUBMITTED']) === $events, 'scope_unaccounted_zero');
...
echo "TOTAL_CANONICAL_AUDIT_EVENT_TYPES=44\n";
echo "EVENT_CATALOG_MATCH=44/44\n";
echo "POLICY_REGISTRY_MATCH=44/44\n";
echo "OPERATION_MAPPING=44/44\n";
echo "SOURCE_MODULE_MAPPING=44/44\n";
