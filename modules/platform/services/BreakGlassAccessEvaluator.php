...
final class BreakGlassAccessEvaluator
{
    ...
    public function evaluate(PrivilegedAccessRequest $request, AuthorizationDecision $authorization, ?AuditTrailPort $audit, string $nowUtc): \Platform\Contracts\PrivilegedAccessDecision
    {
        ...
        if ($request->mode() !== PrivilegedAccessMode::BREAK_GLASS || $request->authorizationPlane() !== AuthorizationPlane::GOVERNANCE_EMERGENCY) return $this->support->decision($request, PrivilegedAccessReason::MODE_MISMATCH, ['mode', 'authorization_plane'], ['break_glass_governance_emergency']);
        if ($request->riskLevel() !== RiskLevel::R3) return $this->support->decision($request, PrivilegedAccessReason::RISK_REQUIREMENT_UNSATISFIED, ['risk'], ['risk_r3']);
        ...
        if (!$request->emergencyConfirmed()) return $this->support->decision($request, PrivilegedAccessReason::EMERGENCY_REQUIRED, ['emergency'], ['emergency_confirmed']);
        ...
        if ($request->caseReference() === null) return $this->support->decision($request, PrivilegedAccessReason::CASE_REQUIRED, ['emergency_case'], ['case_reference']);
        ...
        if (($reason = $this->support->approvalReason($request, 2, $nowUtc)) !== null) return $this->support->decision($request, $reason, ['dual_approval'], [$reason]);
        ...
        if (!$request->postReviewRequired()) return $this->support->decision($request, PrivilegedAccessReason::POST_REVIEW_REQUIRED, ['post_review'], ['post_review_required']);
        ...
        return $this->support->decision($request, PrivilegedAccessReason::POLICY_SATISFIED_NOT_ACTIVATABLE, ['feature_flag', 'actors', 'emergency', 'case', 'reason', 'scopes', 'expiration', 'reauthentication', 'mfa', 'dual_approval', 'visibility', 'post_review', 'authorization', 'audit'], [], true, $auditStatus, 'break_glass_policy_satisfied');
    }
