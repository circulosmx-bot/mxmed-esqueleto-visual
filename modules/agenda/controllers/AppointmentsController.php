...
class AppointmentsController
{
    ...
    public function index(array $params = [])
    {
        ...
        $to = $params['to'] ?? null;
        if (!$from || !$to) {
            return $this->error('invalid_params', 'from and to parameters are required');
        }
        $fromDatetime = $this->parseDatetime($from);
        $toDatetime = $this->parseDatetime($to);
        if (!$fromDatetime || !$toDatetime) {
            return $this->error('invalid_params', 'from/to must be valid datetimes');
        }
    ...
    public function show($id)
    {
        ...
        if (!$id) {
            return $this->error('invalid_params', 'appointment id is required');
        }
        try {
