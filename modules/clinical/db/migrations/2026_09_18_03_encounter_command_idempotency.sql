...
BEGIN
  ...
  ELSE
    SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX) INTO shape_value FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='clinical_encounter_start_requests' AND INDEX_NAME='uq_encounter_start_idempotency';
    IF shape_value<>'doctor_id,idempotency_key' THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='MIGRATION_DRIFT: start idempotency index'; END IF;
  END IF;
  ...
  ELSE
    SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX) INTO shape_value FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='clinical_idempotency_requests' AND INDEX_NAME='uq_clinical_command_idempotency';
    IF shape_value<>'operation_type,doctor_id,context_type,context_id,idempotency_key' THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='MIGRATION_DRIFT: command idempotency index'; END IF;
  END IF;
