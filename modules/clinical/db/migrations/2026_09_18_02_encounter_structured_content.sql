...
BEGIN
  ...
  ELSE
    ...
    SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX) INTO shape_value FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='clinical_encounter_sections' AND INDEX_NAME='uq_encounter_section_concept';
    IF shape_value<>'encounter_id,section_type' THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='MIGRATION_DRIFT: uq_encounter_section_concept'; END IF;
  END IF;
