...
ANALYSIS="$ROOT/api/_lib/clinical_m6_monitoring_analysis.php"
CLI="$ROOT/scripts/clinical-m6-monitor.php"
php -l "$OBS" >/dev/null
php -l "$ANALYSIS" >/dev/null
php -l "$CLI" >/dev/null
python3 - "$ROOT" <<'PY'
from pathlib import Path
import json,sys
r=Path(sys.argv[1]); o=(r/'api/_lib/clinical_m6_observability.php').read_text(); a=(r/'api/_lib/clinical_m6_monitoring_analysis.php').read_text(); c=(r/'scripts/clinical-m6-monitor.php').read_text(); router=(r/'api/clinical/index.php').read_text(); agenda=(r/'modules/agenda/services/ClinicalEncounterBridge.php').read_text(); service=(r/'api/_lib/clinical_multipart_document_service.php').read_text()
