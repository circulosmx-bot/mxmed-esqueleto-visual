"""Real disposable Director QA. No mocked success; see PLAN02BR2_REVIEW.md."""
import json, os, uuid
from pathlib import Path
from playwright.sync_api import sync_playwright, expect

OUT = Path(os.environ['PLAN02BR2_ARTIFACTS']); OUT.mkdir(parents=True, exist_ok=True)
...
def appointment(page,date=None):
    ...
    page.locator('[data-ns-slot]').first.click(); expect(page.locator('.plan02b-modal-content [role=status]')).to_have_text('Horario seleccionado. Aún no reservado. Se reservará al confirmar las acciones.'); assert 'Elige un horario' not in page.locator('.plan02b-modal').inner_text()
...
def followup(page,title):
    ...
    assert page.locator('[data-ns-link]').input_value()=='no'; expect(page.locator('.plan02b-appointment-context')).to_contain_text('Consultorio Norte'); page.locator('[data-ns-link]').select_option('yes'); expect(page.locator('[data-ns-due]')).not_to_be_visible()
...
def confirm(page):
    page.locator('[data-ns="confirm"]').click(); page.wait_for_function('!window.mxmedPlanNextSteps.isBusy()',timeout=30000)
...
def full(browser):
    ...
    order(page,'PLAN02BR2 laboratorio A');order(page,'PLAN02BR2 imagen B');count(page,2)
    collector(page);expect(page.locator('[data-plan02b-collector] h4')).to_have_text('Por confirmar');first=page.locator('[data-prepared]').first;first.locator('[data-review]').click()
    page.locator('[data-order-field="title"]').fill('PLAN02BR2 laboratorio A revisada');page.screenshot(path=str(OUT/'order-modal.png'))
    ...
    page.locator('[data-m7-section="plan"]').click();appointment(page,os.environ['PLAN02BR2_FULL_DATE']);followup(page,'PLAN02BR2 Revisar resultados');count(page,4)
    ...
    confirm(page);expect(page.locator('[data-ns-message]')).to_contain_text('Próximos pasos registrados');count(page,0);expect(page.locator('[data-prepared]')).to_have_count(0);expect(page.locator('[data-ns="confirm"]')).to_have_count(0);assert page.locator('[data-plan02b-collector]').bounding_box()['height']<85;assert len(writes)==4,writes
    ...
    page.locator('[data-m7-doc-refresh]').click();expect(page.locator('[data-m7-encounter-documents]')).to_contain_text('PLAN02BR2 laboratorio A revisada');expect(page.locator('[data-remove]')).to_have_count(0);page.screenshot(path=str(OUT/'collector-success.png'))
    page.reload(wait_until='commit');ready(page);collector(page);count(page,0);expect(page.locator('[data-prepared]')).to_have_count(0);assert saved(page)==s;assert not page.evaluate('window.mxmedPlanNextSteps.hasPending()');page.locator('[data-m7-section="finalize"]').click();decisions=[];page.once('dialog',lambda d:(decisions.append(d.message),d.dismiss()));page.locator('[data-m7-finalize]').click();assert decisions and decisions[0].startswith('Finalizar cerrará');expect(page.locator('.plan02b-leave')).to_have_count(0);assert len(writes)==4;report.update(writes=writes,state=s,appointment=appt,task=task);(OUT/'full-results.json').write_text(json.dumps(report,indent=2,ensure_ascii=False));print('PASS pending 4 → 0, canonical handoff, compact success, restored SUCCESS excluded, finalization unblocked, modals + four widths + navigation',flush=True);ctx.close()
