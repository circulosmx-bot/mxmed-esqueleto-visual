...
qa_db="lon03b_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon03b_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon03b-browser-XXXXXXXX")"
http_pid=""
cleanup() {
...
for user in a b; do
  LON03B_QA_USER="u_$user" LON03B_QA_SESSION="lon03b-$user" php -d "session.save_path=$qa_root/sessions" -r 'session_id(getenv("LON03B_QA_SESSION"));session_start();$_SESSION["doctor_id"]="d_a";$_SESSION["user_id"]=getenv("LON03B_QA_USER");session_write_close();'
done
...
LON03B_QA_BASE="$base" LON03B_QA_ROOT="$root_dir" LON03B_QA_DB="$qa_db" \
  LON03B_QA_WINDOW_PATH="$window_path" python3 "$root_dir/modules/clinical/qa/lon03b_browser_gate.py"
