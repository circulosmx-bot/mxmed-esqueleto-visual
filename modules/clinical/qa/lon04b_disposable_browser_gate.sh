...
qa_db="lon04b_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon04b_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon04b-browser-XXXXXXXX")"
http_pid=""
cleanup() {
...
INSERT INTO clinical_encounter_sections VALUES (201,101,'assessment','Synthetic saved assessment'),(202,102,'assessment','Foreign assessment');
...
MXMED_DB_HOST=localhost MXMED_DB_NAME="$qa_db" MXMED_DB_USER=root MXMED_DB_PASS='' MXMED_LON03A_WRITE_ENABLED=1 MXMED_LON04A_WRITE_ENABLED=1 \
...
LON04B_QA_BASE="$base" LON04B_QA_ROOT="$root_dir" LON04B_QA_DB="$qa_db" LON04B_QA_WINDOW_PATH="$window_path" \
  python3 "$root_dir/modules/clinical/qa/lon04b_browser_gate.py"
