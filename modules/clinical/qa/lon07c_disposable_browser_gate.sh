...
qa_db="lon07c_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon07c_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon07c-qa-XXXXXXXX")"
http_pid=""
cleanup(){
...
INSERT INTO patients_patients VALUES ('p_a'),('p_b'),('p_empty'),('p_fallback');
INSERT INTO patients_doctor_links (doctor_id,patient_id,status) VALUES ('d_a','p_a','active'),('d_b','p_b','active'),('d_a','p_empty','active'),('d_a','p_fallback','active');
...
mysql "$qa_db" <<'SQL'
INSERT INTO clinical_encounters (doctor_id,patient_id,encounter_dt,status,opened_by_user_id) VALUES ('d_a','p_a',UTC_TIMESTAMP(),'open','u_a'),('d_b','p_b',UTC_TIMESTAMP(),'open','u_b'),('d_a','p_fallback',UTC_TIMESTAMP(),'open','u_a');
...
mysql "$qa_db" -e "INSERT INTO clinical_observations (encounter_id,code,value_numeric,unit,effective_at,recorded_at,recorded_by_user_id,source,provenance_json,effective_at_authority) VALUES (3,'weight',91,'kg',UTC_TIMESTAMP(),UTC_TIMESTAMP(),'u_a','direct_measurement','{}','CAPTURE_TIME_FALLBACK')"
...
LON07C_QA_BASE="$base" LON07C_QA_DB="$qa_db" LON07C_QA_ROOT="$root_dir" python3 "$root_dir/modules/clinical/qa/lon07c_browser_gate.py"
