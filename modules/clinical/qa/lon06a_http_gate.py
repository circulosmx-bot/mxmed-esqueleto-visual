...
with sync_playwright() as playwright:
    ...
    sql("INSERT INTO agenda_appointments VALUES ('a_new','d_a','p_a','confirmed','2030-10-01 12:00:00')")
    ...
    check(create(a,'canceled-link',appointment_id='a_a').json()['error']=='INELIGIBLE_APPOINTMENT','canceled appointment cannot be linked')
    first=create(a,'lon06a-create',source_encounter_id=101,appointment_id='a_new',due_at='2026-10-01 12:00:00',responsible_user_id='u_a')
    ...
    check(task['provenance']=='ENCOUNTER_DERIVED_EXPLICIT_ENTRY' and int(task['source_encounter_id'])==101 and task['appointment_id']=='a_new','source and Agenda linkage attributable')
    check(count('clinical_patient_task_audit_events')==1,'create audit committed')
    replay=create(a,'lon06a-create',source_encounter_id=101,appointment_id='a_new',due_at='2026-10-01 12:00:00',responsible_user_id='u_a')
    ...
    changed=create(a,'lon06a-create',title='Changed task',source_encounter_id=101,appointment_id='a_new',due_at='2026-10-01 12:00:00',responsible_user_id='u_a')
    ...
    check(subprocess.run(['mysql',db,'-e',"DELETE FROM agenda_appointments WHERE appointment_id='a_new'"],capture_output=True).returncode!=0,'linked appointment cannot cascade-delete task')
