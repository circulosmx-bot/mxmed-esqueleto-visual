...
def recovery(browser):
    ctx,page=boot(browser);order(page,'PLAN02BR1 collision order');appointment(page,os.environ['PLAN02BR1_COLLISION_DATE']);followup(page,'PLAN02BR1 collision followup');collector(page);count(page,3)
    ...
    confirm(page);s=saved(page);assert s['orders'][0]['state']=='SUCCESS' and s['appointment']['state']=='FAILED' and s['followup']['state']=='BLOCKED_BY_DEPENDENCY';assert len(writes)==2
    ...
    confirm(page);s=saved(page);assert s['orders'][0]['result']['document_id']==orderid and sum('/documents' in r['url'] for r in writes)==1;assert s['followup']['result']['item']['appointment_id']==s['appointment']['result']['appointment_id'];count(page,3)
    ...
    ctx,page=boot(browser);order(page,'PLAN02BR1 recovery order A');order(page,'PLAN02BR1 recovery order B');appointment(page,os.environ['PLAN02BR1_RECOVERY_DATE']);followup(page,'PLAN02BR1 recovery followup');collector(page);lost={};attempts=[]
    def intercept(route):
        req=route.request
    ...
    page.once('dialog',lambda d:d.accept());page.reload(wait_until='commit');ready(page);collector(page);restored=saved(page);assert restored['appointment']['key']==snapshot['appointment']['key'] and restored['appointment']['payload']==snapshot['appointment']['payload'];assert restored['appointment']['uncertain'];count(page,4)
    ...
    assert s['orders'][1]['result']['document_id']==lost['order']['body']['data']['document_id'];assert s['appointment']['result']['appointment_id']==lost['appointment']['body']['data']['appointment_id'];assert s['followup']['result']['item']['task_id']==lost['followup']['body']['data']['item']['task_id'];count(page,4)
