...
qa_db="lon05b_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon05b_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon05b-browser-XXXXXXXX")"
http_pid=""
cleanup() {
...
for user in a b; do
  LON05B_QA_USER="u_$user" LON05B_QA_SESSION="lon05b-$user" php -d "session.save_path=$qa_root/sessions" -r 'session_id(getenv("LON05B_QA_SESSION"));session_start();$_SESSION["doctor_id"]="d_a";$_SESSION["user_id"]=getenv("LON05B_QA_USER");session_write_close();'
done
...
MXMED_DB_HOST=localhost MXMED_DB_NAME="$qa_db" MXMED_DB_USER=root MXMED_DB_PASS='' MXMED_LON03A_WRITE_ENABLED=1 MXMED_LON04A_WRITE_ENABLED=1 MXMED_LON05A_WRITE_ENABLED=1 \
...
LON05B_QA_BASE="$base" LON05B_QA_ROOT="$root_dir" LON05B_QA_DB="$qa_db" LON05B_QA_WINDOW_PATH="$window_path" \
  python3 "$root_dir/modules/clinical/qa/lon05b_browser_gate.py"
