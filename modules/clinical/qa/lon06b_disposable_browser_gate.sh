...
qa_db="lon06b_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon06b_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon06b-qa-XXXXXXXX")"
http_pid=""
cleanup(){
...
for user in a b foreign; do
  ...
  LON06A_QA_DOCTOR="$doctor" LON06A_QA_USER="u_$user" LON06A_QA_SESSION="lon06b-$user" \
...
LON06B_QA_BASE="$base" LON06B_QA_ROOT="$root_dir" LON06B_QA_DB="$qa_db" LON06B_QA_WINDOW_PATH="$window_path" \
  python3 "$root_dir/modules/clinical/qa/lon06b_browser_gate.py"
