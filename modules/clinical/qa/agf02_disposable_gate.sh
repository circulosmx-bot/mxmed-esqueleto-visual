...
qa_db="agf02_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^agf02_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/agf02-qa-XXXXXXXX")"
http_pid=""
cleanup(){
...
for doctor in a b; do
  AGF02_QA_DOCTOR="d_$doctor" AGF02_QA_SESSION="agf02-$doctor" \
    php -d "session.save_path=$qa_root/sessions" -r 'session_id(getenv("AGF02_QA_SESSION"));session_start();$_SESSION["doctor_id"]=getenv("AGF02_QA_DOCTOR");$_SESSION["user_id"]="u_".substr(getenv("AGF02_QA_DOCTOR"),2);session_write_close();'
done
...
AGF02_QA_BASE="$base" AGF02_QA_DB="$qa_db" AGF02_QA_ROOT="$root_dir" python3 "$root_dir/modules/clinical/qa/agf02_disposable_gate.py"
