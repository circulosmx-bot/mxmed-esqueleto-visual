...
qa_db="alr01_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^alr01_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/alr01-qa-XXXXXXXX")"
http_pid=""
cleanup(){
...
INSERT INTO agenda_appointments VALUES ('alr-linked','d_a','p_a','confirmed','2030-10-01 16:30:00');
SQL
...
for doctor in a b; do
  ALR01_QA_DOCTOR="d_$doctor" ALR01_QA_SESSION="alr01-$doctor" \
    php -d "session.save_path=$qa_root/sessions" -r 'session_id(getenv("ALR01_QA_SESSION"));session_start();$_SESSION["doctor_id"]=getenv("ALR01_QA_DOCTOR");$_SESSION["user_id"]="u_".substr(getenv("ALR01_QA_DOCTOR"),2);session_write_close();'
done
...
ALR01_QA_BASE="$base" ALR01_QA_DB="$qa_db" ALR01_QA_ROOT="$root_dir" python3 "$root_dir/modules/clinical/qa/alr01_disposable_gate.py"
