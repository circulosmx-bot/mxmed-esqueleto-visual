...
qa_db="lon06a_m7_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon06a_m7_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon06a-m7-XXXXXXXX")"
http_pid=""
cleanup(){
...
mysql "$qa_db" <<'SQL'
INSERT INTO clinical_encounters (doctor_id,patient_id,encounter_dt,status,opened_by_user_id) VALUES ('d_a','p_a',UTC_TIMESTAMP(),'open','u_a');
...
php -d "session.save_path=$qa_root/sessions" -r 'session_id("lon06a-m7");session_start();$_SESSION["doctor_id"]="d_a";$_SESSION["user_id"]="u_a";session_write_close();'
...
MXMED_DB_HOST=localhost MXMED_DB_NAME="$qa_db" MXMED_DB_USER=root MXMED_DB_PASS='' MXMED_LON06A_WRITE_ENABLED=1 MXMED_CLINICAL_ENCOUNTER_INTEGRITY_V1=1 MXMED_CLINICAL_M6_COHORT_MODE=allowlist MXMED_CLINICAL_M6_COHORT_PAIRS='d_a|p_a' \
...
LON06A_M7_BASE="$base" LON06A_M7_DB="$qa_db" python3 "$root_dir/modules/clinical/qa/lon06a_m7_boundary_gate.py"
