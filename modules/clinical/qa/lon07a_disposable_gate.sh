...
qa_db="lon07a_qa_$(openssl rand -hex 6)"
[[ "$qa_db" =~ ^lon07a_qa_[0-9a-f]{12}$ ]]
qa_root="$(mktemp -d "${TMPDIR:-/tmp}/lon07a-qa-XXXXXXXX")"
http_pid=""
cleanup(){
...
php -d "session.save_path=$qa_root/sessions" -r 'session_id("lon07a-qa");session_start();$_SESSION["doctor_id"]="d_a";$_SESSION["user_id"]="u_a";session_write_close();'
...
LON07A_QA_BASE="$base" LON07A_QA_DB="$qa_db" python3 "$root_dir/modules/clinical/qa/lon07a_http_gate.py"
