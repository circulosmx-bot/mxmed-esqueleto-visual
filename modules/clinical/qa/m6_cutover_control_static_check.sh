...
grep -q "function clinical_m6_cohort_pair_configured" "$LIB"
grep -q "function clinical_m6_cohort_authorized" "$LIB"
grep -q "function clinical_m6_patient_in_any_cohort" "$LIB"
grep -q "function clinical_m6_legacy_write_block_required" "$LIB"
grep -q "return clinical_m6_cohort_pair_configured(\$doctorId, \$patientId);" "$LIB"
grep -q "return clinical_m6_patient_in_any_cohort(\$patientId);" "$LIB"
