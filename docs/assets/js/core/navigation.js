...
function setHash(id, replace){
  ...
  if(replace && history && history.replaceState){
...
function jumpTo(panelId, opts){
  showPanel(panelId, opts);
  ...
  if(!opts || !opts.skipHash) setHash(panelId);
}
...
function selectInfoTab(selector){
  ...
  if(btn){
    ...
    localStorage.setItem('mxmed_info_tab', normalized);
    setHash(normalized.replace(/^#/, ''));
  }
}
function selectPaqTab(selector){
  const btn = document.querySelector('[data-bs-target="'+selector+'"]');
  if(btn){ new bootstrap.Tab(btn).show(); setHash(selector.replace(/^#/, '')); }
}
...
function showPanel(id){
  const skipHash = arguments[1] && arguments[1].skipHash;
  ...
  if(pane){
    ...
    if(!skipHash && id && id.indexOf('p-') === 0) setHash(id);
  }else{
...
function applyHash(){
  const id = getHashId();
  if(!id) return false;
  if(id.indexOf('p-') === 0 && document.getElementById(id)){
    jumpTo(id, { skipHash: true });
    return true;
  }
  const expBtn = document.querySelector('#p-expediente [data-bs-target="#'+id+'"]');
  if(expBtn){
    jumpTo('p-expediente', { skipHash: true });
    try{ new bootstrap.Tab(expBtn).show(); }catch(_){ }
    return true;
  }
  const infoBtn = document.querySelector('#p-info [data-bs-target="#'+id+'"]');
  if(infoBtn){
    jumpTo('p-info', { skipHash: true });
    try{ new bootstrap.Tab(infoBtn).show(); }catch(_){ }
    return true;
  }
  return false;
}
...
// Actualizar hash cuando cambian tabs en expediente y perfil
$(function(){
  document.querySelectorAll('#p-expediente .mm-tabs-row .nav-link').forEach(btn=>{
    btn.addEventListener('shown.bs.tab', ()=>{
      const target = btn.getAttribute('data-bs-target');
      if(target) setHash(target.replace(/^#/, ''));
    });
  });
  document.querySelectorAll('#p-info .nav-link[data-bs-toggle="pill"]').forEach(btn=>{
    btn.addEventListener('shown.bs.tab', ()=>{
      const target = btn.getAttribute('data-bs-target');
      if(target) setHash(target.replace(/^#/, ''));
    });
...
// Cargar contexto activo + panel Manejo hospitalario sin tocar el HTML base.
(function(){
  const loadOnce = (src, match) => {
    try{
      const exists = Array.from(document.scripts || []).some(s => (s.getAttribute('src') || '').includes(match));
      if(exists) return;
  ...
  loadOnce('assets/js/core/context.js', 'core/context.js');
  loadOnce('assets/js/manejo-hospitalario.js', 'manejo-hospitalario.js');
  loadOnce('assets/js/recetas.js', 'recetas.js');
  loadOnce('assets/js/consentimientos.js', 'consentimientos.js');
})();
