...
Pruebas y evidencia: agregar Gate8GPatientIdentityPersistenceMigrationTest.php para validar estáticamente convenciones SQL, pares up/down, privacidad, append-only, idempotencia, locks, retención no inventada, backfill declarativo, rollout deshabilitado y ausencia de dependencias de base de datos en el test. Documentar implementación, rollback y retorno seguro en docs/MXMED_IMPLEMENTACION_V2_PG03_GATE_8G_PERSISTENCIA_MIGRACIONES.md. No declarar PASS si falta un archivo, si se ejecutó SQL, si existe wiring de runtime, si se activa rollout o si aparece una ruta de merge.
...
### PP-314 — Actividad 12: CUT01-B Schedule, Scope and Sentinel Adapters

Fecha: 2026-07-22. Actividad 12. Identificador: `BE-ARCH/MXMed-PG03-CUT01-B-Schedule-Scope-Sentinel-Adapters-01`. Baseline `d492400a6e8c05901d0e2a1065d4657d3065fdd3`; checkpoint Activity 11 `checkpoint/mxmed-product-refinement-v2-activity11`, desreferenciado al baseline; rama `feature/mxmed-pg03-cut01-b-schedule-scope-sentinel-adapters-v2`; commit: SELF — commit atómico de la Actividad 12. Clasificación UI-0.

CUT01-B implementa, sin activar, un alcance exacto de diez archivos: cuatro nuevos y seis modificados. Agrega `CanonicalScheduleReadAdapter`, `CanonicalAvailabilityCompareAdapter`, la prueba CUT01-B y su documento; modifica AgendaSettings, Availability y Waitlist controllers, `ScheduleRepository`, configuración Agenda y este Plan Maestro.

El adapter schedule consume un snapshot de exactamente una de las cinco fuentes legacy ya seleccionada, exige profile, consultorio real, timezone, effective range, duración, gap y versión explícitos, normaliza sólo segundos `00` y delega orden/traslapes al dominio canónico. El adapter compare acepta exclusivamente la respuesta one-day de `AvailabilityController::index()` y entrega un diagnóstico read-only determinista con razones cerradas, diferencias dimensionales y digests SHA-256 sin payload libre.

Los flags `canonical_schedule_read=false` y `canonical_availability_compare=false` quedan implementados, desactivados y sin autorización de activación; R0 permanece disabled y no hay override de request, cliente o environment. Los tres controllers sólo evalúan el flag server-side y conservan una referencia de clase; no instancian ni ejecutan adapters. `ScheduleController`, `AvailabilityRepository` y `AppointmentWriteRepository` permanecen intactos. El sentinel `__all__` conserva la entrada agregada de waitlist, pero nunca alcanza un write, claim, cita o consultorio canónico.

Timezone definitivo, precedencia de fuentes, duración/gap canónicos predeterminados, effective range, registros sin consultorio, backfill y métricas/umbrales/ventanas R1–R4 permanecen `UNRESOLVED_PENDING_PARAMETER_APPROVAL`. No se convierten `America/Mexico_City` ni `30/0` legacy en política canónica.

Validación: `PHP_LINT=8/8`; prueba CUT01-B y catorce regresiones heredadas ejecutadas directamente en la rama, `ACTIVITY12_TESTS=15/15`; Gates 8A–8G permanecen PASS. F-001, F-002, F-004 y F-006 quedan `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; los otros nueve blockers continúan `DECISION_RATIFIED_BLOCKER_OPEN`; `BLOCKERS_OPEN=13/13`. F-026 queda `IMPLEMENTATION_PARTIAL_FLAG_OFF_HIGH_OPEN`. Cutover `NO_GO_BLOCKERS_PRESENT`; readiness `NO_GO_LEGACY_BLOCKERS_PRESENT`.

Evidencia: `/tmp/mxmed-activity12-cut01b-schedule-scope-sentinel-v2/`, exactamente siete JSON y siete TXT con manifiesto SHA-256 estándar. Safe return al parent mediante `git revert --no-commit <ACTIVITY12_COMMIT>` en worktree detached, sin crear commit de rollback y recuperando el tree del baseline.

Impacto: cero conexiones DB, SQL, DDL, migraciones, datos, schedule writes, availability responses, citas, slot claims, waitlist assignments, OTP, backfill, tráfico canónico, comparaciones reales, shadow traffic, rutas, HTTP, payload, UI, Clinical y AWS. Actividad 11 permanece cerrada e integrada; Actividad 12 queda `CUT01_B_IMPLEMENTED_FLAGS_OFF_READY_FOR_POSTVALIDATION_NOT_INTEGRATED`; Actividad 13 bloqueada; contador oficial 11/22; pendientes 11. No se integra, activa, autoriza para R1 ni declara production ready.

### PP-315 — Actividad 13: CUT01-C OTP, DDL Containment and Privacy Boundaries

Fecha: 2026-07-23. Actividad: 13. Identificador: `BE-ARCH/MXMed-PG03-CUT01-C-OTP-DDL-Privacy-Boundaries-01`. Baseline: `9eac1e9c10c2d7a99ff93b1d53319effea2f51a6`. Checkpoint 12: `checkpoint/mxmed-product-refinement-v2-activity12`. Rama: `feature/mxmed-pg03-cut01-c-otp-ddl-privacy-boundaries-v2`. Commit: SELF. Clasificación: UI-0.

CUT01-C implementa, sin activar, un alcance exacto de 31 archivos: 21 nuevos y 10 modificados, incluidos 14 SQL declarativos en siete pares forward/rollback. Ninguna migración fue ejecutada. Las siete superficies auditadas trasladan 17 sentencias DDL legacy fuera de requests: runtime DDL `17 → 0`; el schema se comprueba read-only mediante `information_schema` y falla cerrado con `schema_not_ready`.

El flag `canonical_public_agenda=false` queda implementado, dormido, sin overrides y sin autorización de activación; R0 permanece disabled. `OtpProviderPort` conserva neutralidad y `RejectingOtpProvider` está no configurado, no selecciona proveedor y no envía OTP. Intentos, ventanas, expiración, bloqueo, proveedor, canal, credenciales, SLA y jurisdicción quedan `UNRESOLVED_PENDING_PARAMETER_APPROVAL`; no existen parámetros numéricos canónicos predeterminados.

`PublicOtpRepository` permanece intacto y fuera del wiring canónico. PublicOtp y PublicAppointments sólo conservan una referencia de clase, sin instanciar o ejecutar el adapter. No cambian rutas ni dispatcher. El OTP crudo y destinatarios desaparecen de logs; `debug_code` y `otp_debug` desaparecen de metadata. Seis causas sensibles comparten un error público homogéneo sin enumeración y con referencias opacas.

Validación: tres pruebas CUT01-C y quince regresiones heredadas, `ACTIVITY13_TESTS=18/18`; `PHP_LINT=15/15`. Rollback Git dry-run mediante revert sin commit debe recuperar exactamente el tree del baseline. Impacto: cero conexiones DB, SQL, DDL, migraciones aplicadas, datos, OTP real, rutas, UI, Clinical y AWS.

Findings: F-001 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-002 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-004 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-006 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-012 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-013 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-018 `IMPLEMENTATION_PARTIAL_FLAG_OFF_BLOCKER_OPEN`; F-008 `DECISION_RATIFIED_BLOCKER_OPEN`; F-009 `DECISION_RATIFIED_BLOCKER_OPEN`; F-010 `DECISION_RATIFIED_BLOCKER_OPEN`; F-014 `DECISION_RATIFIED_BLOCKER_OPEN`; F-017 `DECISION_RATIFIED_BLOCKER_OPEN`; F-023 `DECISION_RATIFIED_BLOCKER_OPEN`; F-026 `IMPLEMENTATION_PARTIAL_FLAG_OFF_HIGH_OPEN`. `BLOCKERS_OPEN=13/13`; cutover `NO_GO_BLOCKERS_PRESENT`; readiness `NO_GO_LEGACY_BLOCKERS_PRESENT`.

Actividad 12 permanece cerrada e integrada. Actividad 13 queda `CUT01_C_IMPLEMENTED_FLAG_OFF_READY_FOR_POSTVALIDATION_NOT_INTEGRATED`; Actividad 14 bloqueada; contador oficial `12/22`; pendientes `10`. No se integra al programa, no crea checkpoint y no está lista para R1 o producción.

### PP-316 — Actividad 14: CUT01-D Observability and Clinical Boundary Harness

Fecha: 2026-07-23. Actividad: 14. Identificador: `BE-ARCH/MXMed-PG03-CUT01-D-Observability-Clinical-Boundary-Harness-01`. Parent: `eba2ed0dcc1b11fde4f79e6baeb0901bf17e4093`. Checkpoint 13: `checkpoint/mxmed-product-refinement-v2-activity13`, objeto anotado `a14fa0d8d28919329b8e0ae549c8d043e8d68795`. Rama: `feature/mxmed-pg03-cut01-d-observability-clinical-boundary-harness-v2`. Commit: SELF. Clasificación: UI-0.

CUT01-D implementa, sin activar, un alcance exacto de trece archivos: diez nuevos y tres modificados. Agrega ports de feature flags y observabilidad, adapters canónicos puros para lifecycle e identidad, un placeholder de persistencia rechazante, cuatro harnesses y el documento de implementación; modifica configuración Agenda, `AppointmentWriteRepository` y este Plan Maestro. No existe archivo catorce.

Los once feature flags forman un registro cerrado, aparecen en orden exacto y conservan booleano literal false. Ningún flag es efectivo, no hay override de request, cliente o environment, la activación no está autorizada y el rollout permanece R0 disabled.

El port de observabilidad acepta únicamente referencias opacas seguras. El adapter rechazante no configura sink, no emite métricas y no escribe auditoría. La failure policy deja fail-open sólo la métrica secundaria cuando observabilidad no está disponible; authority audit y write audit fallan cerradas. Sink, retención, owner, on-call, SLO, percentiles, porcentajes, error budgets y ventanas permanecen `UNRESOLVED_PENDING_PARAMETER_APPROVAL`.

El lifecycle adapter delega el plan puro de Gate 8D y proyecta una frontera Agenda→Clinical declarativa mediante digest SHA-256. Agenda conserva ownership y una cita no equivale a un encounter. Schema, retries, DLQ, retención y compensaciones permanecen `UNRESOLVED_PENDING_PARAMETER_APPROVAL`. No existen outbox, saga, worker, queue, DLQ ni requests Clinical.

El patient identity adapter sólo realiza preview mediante el resolver existente y devuelve el mutation plan puro. El placeholder `PdoPatientIdentityPersistenceAdapter` implementa el port protegido sin propiedad o conexión de base de datos y rechaza todas las operaciones con `patient_identity_persistence_not_configured`. No persiste, no ejecuta backfill, no crea ni aplica migraciones, no hace merge y no muta Clinical.

`AppointmentWriteRepository` sólo conserva una referencia local de clase condicionada al flag apagado. No instancia ni ejecuta el adapter y no modifica create, update, cancel, no-show, transacciones, bridge, rutas, contratos HTTP o payloads. `ClinicalEncounterBridge.php`, Gate 8D, Gate 8G, `modules/clinical`, `docs/clinical`, `AuditTrailPort` y `PatientIdentityPersistencePort` permanecen byte-equivalent; superficies protegidas 40/40.

Validación: cuatro pruebas CUT01-D y dieciocho regresiones heredadas, `ACTIVITY14_TESTS=22/22`; `PHP_LINT=11/11`. Safe return mediante revert sin commit en worktree detached temporal, con tree idéntico al parent y sin reset, rebase, amend o force-push.

Impacto: `DATABASE_CONNECTIONS_OPENED=0`; `SQL_EXECUTED=0`; `DDL_EXECUTED=0`; `MIGRATIONS_CREATED=0`; `MIGRATIONS_APPLIED=0`; `DATA_MIGRATED=0`; `PERSISTENCE_WRITES=0`; `BACKFILL_EXECUTED=0`; `METRICS_EMITTED=0`; `AUDIT_EVENTS_WRITTEN=0`; `SHADOW_TRAFFIC_PROCESSED=0`; `CLINICAL_REQUESTS_EXECUTED=0`; `OUTBOX_CREATED=false`; `SAGA_CREATED=false`; `WORKER_CREATED=false`; `QUEUE_CREATED=false`; `DLQ_CREATED=false`; `COMPENSATIONS_IMPLEMENTED=false`; `ROUTE_CHANGES=0`; `HTTP_CONTRACT_CHANGES=0`; `PAYLOAD_CHANGES=0`; `UI_CHANGES=0`; `CLINICAL_RUNTIME_CHANGES=0`; `AWS_WRITES=0`.

Findings: los trece blockers continúan abiertos, `BLOCKERS_OPEN=13/13`; cutover `NO_GO_BLOCKERS_PRESENT`; readiness general `NO_GO_LEGACY_BLOCKERS_PRESENT`. Actividad 13 permanece cerrada e integrada. Actividad 14 queda `CUT01_D_IMPLEMENTED_FLAGS_OFF_READY_FOR_POSTVALIDATION_NOT_INTEGRATED`, no integrada y no declarada para producción. Actividad 15 permanece bloqueada. Contador oficial `13/22`; pendientes `9`. No se crea checkpoint de Actividad 14.

### PP-317 — Actividad 15: CUT-02 Shadow and Audit-Only Scope, Decisions and Readiness

Fecha: 2026-07-23. Actividad: 15. Identificador: `ARCH/MXMed-PG03-CUT02-Shadow-Audit-Only-Scope-Decisions-Readiness-01`. Parent: `fa95d7af6c00adb7e2c5070b3279d036d9ff3fcc`. Checkpoint 14: `checkpoint/mxmed-product-refinement-v2-activity14`, objeto anotado `fffa278f7d183877ea1a9eb2b7e18bc9bf09d618`, desreferenciado al parent. Rama: `architecture/mxmed-pg03-cut02-shadow-audit-readiness-v2`. Commit: SELF. Clasificación: UI-0.

La Actividad 15 completa un alcance exclusivamente documental de tres archivos: dos nuevos y uno modificado. Crea `docs/MXMED_ARQUITECTURA_V2_PG03_CUT02_SHADOW_AUDIT_READINESS.md` y `docs/MXMED_DECISIONES_PROPUESTAS_V2_PG03_CUT02.md`; modifica únicamente este Plan Maestro. No existe archivo cuarto.

Se registran exactamente ocho decisiones DEC-015A–H sobre superficies/secuencia shadow, invariancia legacy, privacidad y auditoría sanitizada, sink/health/readiness/failure policy, métricas/dimensiones, baseline/sampling/ventanas/budgets, owners/on-call/retención/runbooks y hard stops/kill switches/safe return. Todas permanecen `PROPOSED_PENDING_DIRECTOR_APPROVAL`; cero decisiones están aprobadas.

Los once feature flags continúan en booleano literal false. Primera ola futura candidata no autorizada: `canonical_actor_authority`, `canonical_schedule_read`, `canonical_availability_compare`, `canonical_appointment_lifecycle` y `canonical_patient_identity`. Condicionales y bloqueados: `canonical_public_agenda` por privacidad/OTP/Seguridad y `shadow_audit` por sink/retención/health/readiness/owner/on-call/failure policy. Fuera de la primera implementación CUT-02: `patient_identity_persistence`, `legacy_write_disable`, `read_compare` y `backfill`.

Sink, retención, owner, on-call, SLO, p95, p99, error budgets, observation windows, sampling/key/scope/exclusions, plataformas y rutas de dashboard/alerta, thresholds de hard stop, timeouts, residencia, jurisdicción, key management, algoritmo/rotación de referencias opacas, schema de auditoría, retención de evidencia, RTO y RPO permanecen `UNRESOLVED_PENDING_PARAMETER_APPROVAL`; percentiles, porcentajes y duración también permanecen `UNRESOLVED_PENDING_BASELINE_AND_DIRECTOR_APPROVAL`.

CUT-02 no se implementa ni activa. `CUT02_IMPLEMENTATION_AUTHORIZED=false`; `R1_ACTIVATION_AUTHORIZED=false`; `R2_ACTIVATION_AUTHORIZED=false`; R0 disabled. No se selecciona/configura sink, no se implementan dashboards/alertas, no se aprueban sampling/thresholds/retención/owners/on-call y no se procesa tráfico shadow.
...
Validación requerida: estructura arquitectónica 47/47; decisiones 8/8 con estado propuesto 8/8; PP-310–PP-316 byte-equivalent; PP-317 único; flags 11/11 false y runtime true flags 0; pruebas 22/22; lint 11/11; manifests protegidos y de fuentes sin mismatches; safe return por revert del commit documental en worktree detached con tree idéntico al parent; puertos de control sin iniciar servicios.

Los trece blockers permanecen abiertos: `BLOCKERS_OPEN=13/13`; cutover `NO_GO_BLOCKERS_PRESENT`; readiness general `NO_GO_LEGACY_BLOCKERS_PRESENT`. Actividad 14 está cerrada e integrada. `ACTIVITY15=CUT02_DOCUMENTATION_COMPLETE_READY_FOR_POSTVALIDATION_NOT_INTEGRATED`; Actividad 15 queda documentalmente completa, lista para postvalidación y no integrada. Actividad 16 permanece bloqueada. Contador oficial `14/22`; pendientes `8`. No se crea checkpoint 15.

### PP-318 — Actividad 16: CUT-02 Director Decisions Approval

Fecha: 2026-07-23. Actividad: 16. Identificador: `ARCH/MXMed-PG03-CUT02-Director-Decisions-Approval-01`. Parent: `0792e6707f63aadde10cb859534d78d23192e894`. Checkpoint 15: `checkpoint/mxmed-product-refinement-v2-activity15`, objeto anotado `2eaf8059d34818d24b2c0bbecd327f76b31d9e15`, desreferenciado al parent. Rama: `architecture/mxmed-pg03-cut02-director-decisions-approval-v2`. Commit: SELF. Clasificación: UI-0.

La Actividad 16 ratifica, sin implementar ni activar, las recomendaciones técnicas postvalidadas de DEC-015A, DEC-015B, DEC-015C, DEC-015D, DEC-015E, DEC-015F, DEC-015G y DEC-015H. Las ocho decisiones quedan `APPROVED_WITH_DEFERRED_PARAMETERS`: decisiones aprobadas con parámetros diferidos `8`; decisiones pendientes de aprobación directorial `0`; parámetros diferidos presentes. El alcance versionado exacto es de dos archivos modificados y cero nuevos: `docs/MXMED_DECISIONES_PROPUESTAS_V2_PG03_CUT02.md` y este Plan Maestro. `docs/MXMED_ARQUITECTURA_V2_PG03_CUT02_SHADOW_AUDIT_READINESS.md` permanece protegido e intacto; no existe archivo tercero.

Permanecen no aprobados sink, provider, sampling/key/scope/exclusions, observation windows, p95, p99, SLO, error budgets, retención, owners, on-call, plataformas y rutas de dashboard/alerta, thresholds numéricos de hard stop, health/readiness timeouts, residencia, jurisdicción, key management, algoritmo/rotación de referencias opacas, schema de auditoría, retención de evidencia, RTO, RPO y demás valores diferidos. Se conservan `UNRESOLVED_PENDING_PARAMETER_APPROVAL` y `UNRESOLVED_PENDING_BASELINE_AND_DIRECTOR_APPROVAL`; no se fijan cifras, proveedores, personas, porcentajes, percentiles, días ni ventanas.

La ratificación no autoriza implementación o activación: `CUT02_IMPLEMENTATION_AUTHORIZED=false`; `R1_ACTIVATION_AUTHORIZED=false`; `R2_ACTIVATION_AUTHORIZED=false`; `FEATURE_FLAG_ACTIVATION_AUTHORIZED=false`. Los once feature flags permanecen en booleano literal false, `FEATURE_FLAGS=11/11`, `FEATURE_FLAGS_FALSE=11/11`, `RUNTIME_TRUE_FLAGS=0`, y el rollout efectivo continúa R0 disabled. Sink no seleccionado/configurado; dashboards y alertas no implementados; sampling, thresholds, retención shadow, owners y on-call no aprobados.
