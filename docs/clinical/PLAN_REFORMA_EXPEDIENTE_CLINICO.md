...
NEXT_AUTHORIZED_STEP=Director/assistant final review of ratified Encounter Integrity contract and decision whether to authorize a separate physical implementation/validation design chapter.
...
- [ ] **PHASE 2 — ENCOUNTER INTEGRITY** · `IN_PROGRESS`, [CONTRACT01](EXPEDIENTE_ENCOUNTER_INTEGRITY_CONTRACT.md) con D1–D5 ratificadas y pendiente aceptación final del contrato. Contiene inicio, guardado, reanudación, finalización, anulación, lectura histórica, enmiendas, relación documental y plan de prueba controlada; la implementación sigue pendiente. **Salida futura:** dos o más consultas del mismo paciente se crean y revisan de forma independiente, recuperable y doctor-scoped, sin sobrescritura histórica.
...
El [contrato de integridad de consulta](EXPEDIENTE_ENCOUNTER_INTEGRITY_CONTRACT.md) documenta el alcance comprobado en fuente y sus brechas. El Director ratificó D1–D5: una OPEN por médico/paciente con reanudación multioperador autorizado, `VOIDED / ANULADA` auditada, enmienda explícita de CLOSED, vínculo de cita fijo históricamente y representación híbrida de enmiendas. El plan contiene 24 escenarios sintéticos **no ejecutados**. La búsqueda actual de OPEN aún incluye al usuario que abrió; concurrencia, estado de creación controlado por cliente y cierre completamente idempotente continúan como riesgos de implementación. La lectura clínica con posible DDL bloquea validación física segura. CONTRACT01 espera **aceptación final** y no aprueba cambios de esquema, API, UI, datos ni pruebas con escrituras. `NEXT_AUTHORIZED_STEP` es su revisión final y decidir si se autoriza otro capítulo de diseño físico/validación.
