...
create_payload=$(cat <<EOF
{
  ...
  "created_by_id": "qa",
  "patient_notes": "QA read fixture"
}
EOF
)

print_header "Create appointment for read list"
...
from="${DATE} 00:00:00"
to="${DATE} 23:59:59"
print_header "GET appointments list"
url_encode() {
  python3 -c "import urllib.parse,sys; print(urllib.parse.quote(sys.argv[1]))" "$1"
}
from_enc=$(url_encode "$from")
to_enc=$(url_encode "$to")
list_resp=$(curl_request -X GET "$API_BASE/appointments?from=${from_enc}&to=${to_enc}&doctor_id=$DOCTOR_ID&consultorio_id=$CONSULTORIO_ID&limit=50")
echo "$list_resp" | jq .
count=$(echo "$list_resp" | jq -r '.data | length')
if [[ "$count" -lt 1 ]]; then
  fail_assert "appointment list empty"
fi
contains=$(echo "$list_resp" | jq -r --arg id "$(echo "$create_resp" | jq -r '.data.appointment_id')" '[.data[] | select(.appointment_id == $id)] | length')
if [[ "$contains" != "1" ]]; then
  fail_assert "created appointment missing in list"
fi
