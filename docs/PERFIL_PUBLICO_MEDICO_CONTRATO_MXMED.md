...
## Adenda PP-Decisiones 134 - Cierre del readiness de integración del mapper en app.js

### Objetivo de cierre

Esta adenda cierra documentalmente el plan de integración futura del mapper dedicado `window.MXMedSubscriptions` dentro del bloque local de suscripciones en `assets/js/app.js`.

Esta microfase no modifica `assets/js/app.js`, no modifica `assets/js/subscription-messages.js`, no modifica `index.html`, no modifica backend, no ejecuta SQL y no ejecuta HTTP/POST o curl.

### Decisión cerrada

La futura integración del mapper en `assets/js/app.js` debe ser incremental, mínima y limitada inicialmente a mensajes de suscripciones ya existentes.
...
No debe mostrar `supportHint` ni código técnico a audiencia `user`.

### Wrapper local cerrado
...
- consultar de forma segura `window.MXMedSubscriptions.mapActivationError` si existe;
- pasar `code`, `httpStatus`, `audience`, `context` y `fallback`;
- devolver `message`, `severity`, `retryable`, `supportHint` y `exposeCode`;
- conservar fallback local si el mapper no existe;
- proteger la UI si `assets/js/subscription-messages.js` no carga;
- no introducir dependencia dura del namespace;
- no exponer `supportHint` a usuario final;
- no exponer código técnico a audiencia `user`.

### Integración mínima futura

La primera implementación debe limitarse a `devWriteStatusMessage`.
...
La integración debe mantener fallback actual para cualquier código no cubierto.

No debe reemplazar completa la lógica de `devWriteStatusMessage` en una sola microfase.

No debe cambiar `setDevStatusClass` salvo que una microfase explícita lo autorice.

No debe cambiar asignaciones visuales fuera de mensajes equivalentes.

### applyReadOnlyError preservado

`applyReadOnlyError` debe conservar su lógica actual por HTTP status.

Sólo podrá usar el mapper cuando exista `payload.error.code` en una microfase futura explícita.

No debe reemplazarse todavía.

No debe depender del mapper para mantener el panel de suscripción en modo lectura.

### Riesgos cerrados

Riesgos documentados para la implementación futura:

- `assets/js/app.js` es grande y sensible;
- cambios amplios pueden introducir regresiones visuales;
- reemplazar completo `devWriteStatusMessage` podría alterar mensajes actuales;
- `applyReadOnlyError` trabaja principalmente por HTTP status;
- el panel DEV/local no debe confundirse con flujo real;
- no debe introducirse dependencia dura del namespace;
- no debe exponerse `supportHint` a usuario final;
- no debe exponerse código técnico a audiencia `user`;
- no se debe conectar `activate-after-payment`;
- no se debe introducir `payment_event_uuid`.
...
- modificar sólo `assets/js/app.js` y documentación;
- crear wrapper local de lectura segura del mapper;
- usar mapper sólo en `devWriteStatusMessage` para códigos controlados;
- mantener fallback actual;
- no tocar `index.html`;
- no tocar `assets/js/subscription-messages.js`;
- no tocar `assets/js/messages.js`;
- no tocar backend;
- no conectar `activate-after-payment`;
- no agregar `payment_event_uuid`;
- ejecutar QA estática posterior.
...
- PP-Decisiones 131: cierre de módulo dedicado;
- PP-Decisiones 132: implementación mínima de `assets/js/subscription-messages.js`;
- PP-Decisiones 133: readiness de integración futura del mapper en `assets/js/app.js`.
...
- `confirm_mock` sólo confirma evidencia de pago mock/dev;
- `activate-after-payment` sigue sin conectarse al frontend;
- `assets/js/app.js` todavía no consume el mapper en esta microfase;
...
## PP-Decisiones 140 - Cierre readiness de state read-model para activacion post-pago
...
DOCS / Cierre documental del readiness de state read-model para flujo frontend de activacion post-pago.

### Objetivo

Esta adenda cierra documentalmente la decision de readiness del state read-model/backend activation state requerido antes de cualquier implementacion de UI o POST desde frontend.
...
- PP-Decisiones 138: readiness del flujo frontend de activacion post-pago;
- QA current-state read-only: decision B, el frontend actual no tiene datos suficientes;
- PP-Decisiones 139: readiness de state read-model para activacion post-pago.

La decision base queda cerrada:

El flujo frontend de activacion post-pago requiere primero un state read-model/backend activation state.

### Hallazgos preservados

El frontend actual no tiene:
...
El frontend actual si tiene:

- `entity_type` y `entity_id` via `context/current`;
- active subscription inferible desde el read-model actual;
- `Idempotency-Key` solo para write DEV `/subscriptions`.

El frontend actual no consume:
...
### Opcion recomendada cerrada

Queda cerrada como preferente la Opcion B: endpoint especifico de activation state.

Motivo:

- separa estado transaccional de checkout/pago del read-model de suscripcion actual;
- evita contaminar `current`;
- entrega `payment_event_uuid` desde backend confiable;
- permite calcular `activation_eligibility.can_activate` server-side;
- permite devolver reasons seguros para UI;
- evita habilitar POST con datos incompletos.

La primera exposicion recomendada debe ser DEV/local o soporte/admin, no usuario final publico.

### Rutas conceptuales cerradas

Ruta conceptual por entidad:

```text
GET /api/subscriptions/index.php/entities/{entity_type}/{entity_id}/payment-activation-state
```

Ruta conceptual con checkout explicito:
...
- si existe un unico checkout candidato activo/pendiente por entidad, puede evaluarse ruta por entidad;
- si hay mas de un checkout posible, debe preferirse ruta con `checkout_intent_uuid` explicito;
- esta microfase no decide la ruta final de implementacion;
- la ruta final debe validarse despues de revisar cobertura de repositorios.

### Datos minimos cerrados

El state read-model debe considerar estas secciones:
...
- `entity_id`;
- `scope_valid`;
- checkout `uuid`, `status`, `plan_code`, `billing_period`, `amount_cents`, `currency`, `expires_at`, `subscription_id`, `contract_acceptance_uuid`;
- payment intent `uuid`, `checkout_intent_uuid`, `provider`, `normalized_status`, `provider_status`, `paid_at`;
- payment event `uuid`, `payment_intent_uuid`, `checkout_intent_uuid`, `event_type`, `processing_status`, `processed_at`;
- contract acceptance `uuid`, `status`, `subscription_id`;
- active subscription `exists`, `subscription_id`, `status`, `plan_code`;
- activation eligibility `can_activate`, `reasons`, `required_action`;
- idempotency `suggested_key_prefix` o `key_strategy`, `replay_safe`;
- UI `recommended_label`, `recommended_message_code`, `severity`, `retryable`.
...
- checkout intent existe;
- checkout intent esta `pending_payment`;
- checkout intent pertenece al scope de entidad;
- payment intent existe;
- payment intent pertenece al checkout;
- payment intent esta `paid`;
- `provider_status` es compatible con `mock_paid` o equivalente futuro;
- payment event existe;
- payment event pertenece al payment intent y checkout;
- payment event esta `processed`;
- payment event es `payment_intent_confirm`;
- contract acceptance existe;
- contract acceptance esta `accepted_pending_payment`;
- contract acceptance tiene `subscription_id` compatible con la regla backend actual;
- no existe active subscription vigente;
- sesion/autorizacion es valida.

### Reasons cerradas

Reasons seguras cerradas:
...
Estas reasons no deben incluir SQL, stacktrace, provider secrets, hashes, payload sensible ni informacion de otra entidad.
...
- pertenencia checkout/payment/payment_event;
- visibilidad del usuario actual;
- ausencia de filtrado cross-entity.

### Relacion frontend cerrada

El frontend futuro solo debe:

- leer este estado;
- mostrar estado;
- habilitar o deshabilitar accion;
- generar `Idempotency-Key` si `can_activate=true`;
- ejecutar POST solo en microfase futura explicita.

El frontend no debe:

- inferir `payment_event_uuid`;
- buscarlo en DOM oculto no confiable;
- inventarlo;
- activar si `can_activate=false`;
- mezclar `confirm_mock` y `activate-after-payment` sin decision explicita.

### Alcance fuera de esta decision

Este cierre no crea endpoint.
...
Antes de implementar endpoint real, se debe validar read-only si los repositorios actuales ya permiten leer:
...
## PP-Decisiones 142 - Cierre readiness de metodos read-only para state read-model post-pago
...
DOCS / Cierre documental del readiness de metodos read-only para state read-model post-pago.

### Objetivo

Se cierra documentalmente la decision iniciada en:

```text
BE/SPEC-Suscripciones-PaymentIntent-PostPaymentActivation-StateReadModel-RepositoryMethods-Readiness-01
```

El cierre confirma que el state read-model futuro `payment-activation-state` requiere primero metodos PHP read-only minimos en repositorios existentes.

Esta decision no implementa PHP, endpoints, servicios, JS, HTML ni cambios frontend.

No ejecuta SQL, HTTP, POST ni curl.

No modifica schema, seeds ni fixtures.
...
- PP-Decisiones 140: cierre readiness de state read-model para activacion post-pago;
- QA repository coverage read-only: cobertura parcial de repositorios para construir eligibility;
- PP-Decisiones 141: readiness de metodos read-only faltantes para state read-model post-pago.

La decision queda cerrada como preparatoria para una implementacion PHP read-only controlada.

### Metodos cerrados para SubscriptionPaymentEventRepository

Metodo minimo preferente:

```php
findProcessedConfirmByPaymentIntentUuid(string $paymentIntentUuid): ?array
```

Metodo alternativo u opcional:
...
- devolver solo evento de confirmacion procesado;
- exigir `event_type = payment_intent_confirm`;
- exigir `processing_status = processed`;
- respetar `deleted_at IS NULL` si aplica;
- validar pertenencia al payment intent y, si aplica, al checkout;
- no crear eventos;
- no cambiar estado;
- no exponer raw payload sensible;
- no devolver datos cross-entity.

### Metodos cerrados para SubscriptionContractAcceptanceRepository
...
Metodo adicional candidato:

```php
findPendingPaymentByEntity(string $entityType, int $entityId): ?array
```

Metodo opcional si existe relacion directa con checkout:
...
- devolver solo aceptacion con `status = accepted_pending_payment`;
- respetar entidad y scope;
- permitir validar `subscription_id`;
- permitir validar snapshot contractual;
- no crear aceptacion;
- no modificar aceptacion;
- no ejecutar `linkSubscriptionId`;
- no activar suscripcion.

### Metodo cerrado para SubscriptionCheckoutIntentRepository

El metodo actual queda aceptado para checkout explicito:

```php
findByUuid(string $uuid): ?array
```

Metodo opcional si se aprueba endpoint por entidad sin checkout uuid:
...
- solo aplica si una ruta futura permite consultar state por entidad sin `checkout_intent_uuid`;
- ante multiples candidatos, debe devolver estado ambiguo o usar un orden estable documentado;
- no debe inventar seleccion insegura;
- no debe marcar checkout como `activated`;
- no debe poblar `subscription_id`;
- no debe ejecutar writes.

### Resolver de entidad y scope cerrado

Contrato conceptual cerrado:
...
- `SubscriptionEntityResolverService`; o
- resolver read-only dedicado para payment activation state.

Reglas cerradas:

- validar `entity_type`;
- validar y normalizar `entity_id`;
- validar scope de sesion;
- no asumir checkout escribible;
- no ejecutar writes;
- no consultar datos innecesarios;
- no filtrar datos de otra entidad.

### Servicio futuro documentado

Servicio futuro habilitado por esta decision:

```php
BuildSubscriptionPaymentActivationStateService::build(array $input): array
```

Responsabilidades futuras:

- agregar checkout intent;
- agregar payment intent;
- agregar payment event procesado;
- agregar contract acceptance pendiente;
- agregar active subscription vigente;
- calcular `can_activate`;
- calcular `reasons`;
- preparar metadata UI segura;
- preparar estrategia segura de idempotencia.

El servicio futuro no debe:

- llamar `ActivateSubscriptionAfterPaymentService` como write;
- activar suscripcion;
- crear `profile_subscriptions`;
- modificar checkout;
- modificar payment intent;
- crear payment events;
- escribir idempotency keys;
- depender de POST;
- exponer datos de otra entidad.

### Seguridad cerrada

Ningun metodo read-only ni el futuro servicio de state debe devolver:
...
- datos personales no necesarios;
- datos de otra entidad;
- IDs internos autoincrementales cuando no sean necesarios para UI.

La conversion a payload frontend debe quedar fuera de los repositorios y vivir en servicio/capa de response futura.

### Sin SQL/schema

El cierre confirma que no se requiere SQL/schema para la siguiente etapa.
...
La siguiente implementacion debe limitarse a metodos PHP read-only minimos, sin endpoint y sin servicio completo.
